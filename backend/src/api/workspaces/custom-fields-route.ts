 ...
 */
router.get("/", checkWorkspaceRole("viewer"), async (req: any, res) => {
  try {
 ...
 */
router.post("/", checkWorkspaceRole("admin"), async (req: any, res) => {
  try {
    const { workspaceId } = req.params;
 ...
 */
router.delete("/definitions/:fieldId", checkWorkspaceRole("admin"), async (req: any, res) => {
  try {
