 ...
 *
 * Walks a CDM and produces a `CrossReferenceIndex`: every figure/table/equation
 * gets a display number (`Figure 1`) assigned by document order, plus its
 * location. It also runs the validation checks the spec calls out:
 *   - an object exists but is never mentioned in the prose,
 *   - the prose mentions e.g. "Figure 8" but no such figure exists,
 *   - a citation has no matching reference (broken reference),
 *   - a reference is never cited,
 *   - missing DOI / year / duplicate reference ids.
 *
 * Pure and synchronous (uses `cdmToPlainText` for prose scanning) so it is easy
 * to unit-test.
 */
import type {
  BlockNode,
...
function captionText(
  caption: BlockNode[] | undefined,
  doc: CanonicalDocument,
): string {
  if (!caption || caption.length === 0) return "";
  const text = cdmToPlainText({ ...doc, body: caption, references: [], assets: [] });
  return text.replace(/\s+/g, " ").trim();
}

function collectCitations(nodes: InlineNode[], acc: string[]): void {
...
function collectBlockCitations(blocks: BlockNode[], acc: string[]): void {
  for (const b of blocks) {
    switch (b.type) {
      ...
      case "bulletList":
      case "orderedList":
        for (const item of b.items) collectBlockCitations(item.content, acc);
        break;
      case "blockquote":
      case "appendix":
      ...
      case "table":
        for (const row of b.rows)
          for (const cell of row.cells) collectBlockCitations(cell.content, acc);
        break;
      case "figure":
        if (b.caption) collectBlockCitations(b.caption, acc);
        break;
      default:
        break;
    }
...
interface EntryBuilders {
  figures: XrefEntry[];
  tables: XrefEntry[];
  equations: XrefEntry[];
}

function buildEntries(doc: CanonicalDocument): EntryBuilders {
  const figures: XrefEntry[] = [];
  const tables: XrefEntry[] = [];
  const equations: XrefEntry[] = [];
  ...
  const walk = (blocks: BlockNode[], depth: number): void => {
    blocks.forEach((b, idx) => {
      ...
      switch (b.type) {
        case "bulletList":
        case "orderedList":
          for (const item of b.items) walk(item.content, depth + 1);
          break;
        case "blockquote":
        case "appendix":
        ...
        case "table":
          for (const row of b.rows)
            for (const cell of row.cells) walk(cell.content, depth + 1);
          break;
        case "figure":
          if (b.caption) walk(b.caption, depth + 1);
          break;
        default:
          break;
      }
...
function checkReferences(
  ...
  citedIds: Set<string>,
  findings: ExportFinding[],
): CrossReferenceIndex["references"] {
  ...
  return refs.map((r) => {
    ...
    const csl = r.cslJson as Record<string, unknown> | undefined;
    const missingYear =
      !!csl && csl.year === undefined && csl.issued === undefined;
...
export function buildCrossReferenceIndex(doc: CanonicalDocument): XrefResult {
