/**
 * Canonical Document Model -> Markdown (GFM). Used by the Markdown output adapter.
 */
      ...
      switch (node.type) {
        ...
        case "orderedList":
          return node.items
            .map(
              (i, idx) =>
                `${idx + 1}. ${renderBlocks(i.content, depth + 1, placeholderLabels)}`,
            )
            .join("\n");
        case "blockquote":
