...
function asText(nodes: TiptapNode[] | undefined): string {
  if (!nodes) return "";
  return nodes
    .map((n) => {
      if (typeof n.text === "string") return n.text;
      if (n.content) return asText(n.content);
      return "";
    })
    .join("");
}
...
function convertMark(mark: TiptapMark): InlineMark | null {
  switch (mark.type) {
    case "bold":
    case "italic":
    case "underline":
...
function convertInline(nodes: TiptapNode[] | undefined): InlineNode[] {
...
function convertListItem(node: TiptapNode): ListItem {
  const blocks = (node.content ?? []).map(convertBlock).filter(
    (b): b is BlockNode => b !== null,
  );
  return { type: "listItem", content: blocks };
}

function findImage(nodes: TiptapNode[]): TiptapNode | undefined {
  for (const n of nodes) {
    if (IMAGE_NODE_TYPES.has(n.type)) return n;
    if (n.content) {
...
function findCaption(nodes: TiptapNode[]): BlockNode[] | undefined {
  for (const n of nodes) {
...
function convertFigure(
  ...
  if (image) {
    ...
    if (src) {
      ...
      figure.width = image.attrs?.width as string | number | undefined;
    }
  } else {
    findings.push({
      severity: "warning",
  ...
  if (str(node.attrs?.span)) figure.span = str(node.attrs?.span);
  if (str(node.attrs?.style)) figure.style = str(node.attrs?.style);
  return figure;
}
...
function convertImageToFigure(
  ...
  if (src) {
    ...
    figure.width = node.attrs?.width as string | number | undefined;
  } else {
    findings.push({
      severity: "warning",
 ...
 */
function convertBlock(node: TiptapNode): BlockNode | null {
  switch (node.type) {
    case "heading":
      return {
        type: "heading",
        level: (num(node.attrs?.level) as 1 | 2 | 3 | 4 | 5 | 6) ?? 1,
...
export function tiptapToCdm(
  input: TiptapDoc | unknown,
  ...
  const walkInline = (nodes: InlineNode[], blockIndex: number) => {
    for (const n of nodes) {
  ...
  body.forEach((b, i) => {
    if (b.type === "heading" || b.type === "paragraph") walkInline(b.content, i);
    if (b.type === "blockquote") b.content.forEach((c) => {
      if (c.type === "heading" || c.type === "paragraph") walkInline(c.content, i);
    });
  });
