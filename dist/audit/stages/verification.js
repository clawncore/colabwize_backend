...
exports.VerificationStage = void 0;
const uuid_1 = require("uuid");
const citationMatcher_1 = require("../../services/citationAudit/citationMatcher");
const externalVerification_1 = require("../../services/citationAudit/externalVerification");
/**
...
exports.VerificationStage = {
    ...
    execute: async (job, context) => {
        ...
        const formattedInline = citations.map((cit) => ({
            text: cit.text,
            start: cit.start,
            end: cit.end,
        ...
        const pairs = citationMatcher_1.CitationMatcher.matchCitations(formattedInline, formattedRefs, style);
        ...
        if (prismaClient.auditJob && prismaClient.citationPairHash) {
            try {
                ...
                if (previousAudit) {
                    ...
                    prevHashes.forEach((h) => previousHashes.add(h.pair_hash));
        ...
        const verificationResults = await externalVerification_1.ExternalVerificationService.verifyCitationPairs(pairs, context.userId, {
        ...
        if (prismaClient.citationPairHash) {
            try {
                const currentHashes = pairs.map((pair) => ({
                    ...
                    pair_hash: externalVerification_1.ExternalVerificationService.computePairHash(pair),
                ...
                await prismaClient.$transaction([
                    ...
                    ...currentHashes.map((h) => prismaClient.citationPairHash.upsert({
                        where: {
        ...
        const extractedMetas = bibliography.map((ref, idx) => ({
            index: idx,
            title: citationMatcher_1.CitationMatcher.extractTitle(ref.text)?.toLowerCase().trim(),
            doi: citationMatcher_1.CitationMatcher.extractDOI(ref.text)?.toLowerCase().trim(),
            year: citationMatcher_1.CitationMatcher.extractYear(ref.text)
        }));
        ...
        formattedRefs.forEach((ref, index) => {
            ...
            const hasDoi = !!citationMatcher_1.CitationMatcher.extractDOI(sourceText);
        ...
        job.report.issues.forEach((issue) => {
            if (!issue.category) {
                if (issue.type === 'UNVERIFIED_SOURCE')
