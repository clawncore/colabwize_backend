...
exports.RetractionCheckStage = void 0;
const uuid_1 = require("uuid");
const retractionWatchService_1 = require("../../services/retractionWatchService");
/**
...
exports.RetractionCheckStage = {
    ...
    execute: async (job, context) => {
        ...
        for (const result of verificationResults) {
            if (!result.foundPaper)
                continue;
            if (result.foundPaper.isRetracted) {
                const knownRetraction = {
                    doi: "",
                    info: {
                        isRetracted: true,
        ...
        const retractionResults = await retractionWatchService_1.RetractionWatchService.checkDOIs(dois);
        const flags = [];
        ...
        console.log(`[Stage] RETRACTION_CHECK: Checked ${dois.length} DOIs, found ${flags.length} with retraction notices.`);
    },
};
function extractDOIFromResult(result) {
...
function addRetractionIssue(job, flag) {
