...
exports.registerStage = registerStage;
...
const PIPELINE_STAGES = stages_1.ALL_STAGES;
/**
 ...
 */
function createInitialReport(jobAuthIds, style = "APA") {
    return {
        metadata: {
 ...
 */
function startAudit(documentId, projectId, docState, style = "APA", userId = "") {
    ...
    (0, persistence_1.persistAuditJobInBackground)(job);
 ...
 */
async function runPipeline(auditId, docState, userId) {
    ...
    try {
        for (const stage of PIPELINE_STAGES) {
            ...
            (0, persistence_1.persistAuditJobStatusInBackground)(job);
            ...
            try {
                await Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error(`Stage "${stage.name}" timed out after ${STAGE_TIMEOUT_MS / 1000}s`)), STAGE_TIMEOUT_MS)),
                ]);
