...
class GrobidService {
    ...
    static async processPDF(pdfBuffer, fileName = "document.pdf") {
        const hash = this.computeHash(pdfBuffer);
        const cached = this.cache.get(hash);
        if (cached && Date.now() - cached.timestamp < this.CACHE_TTL_MS) {
            logger_1.default.info("[PDF-PARSE] Returning cached result for", { fileName });
            return cached.result;
        }
        try {
            logger_1.default.info("[PDF-PARSE] Processing PDF", {
                fileName,
                size: pdfBuffer.length,
            });
            const data = await pdfParse(pdfBuffer);
            const text = data.text || "";
            logger_1.default.info("[PDF-PARSE] Extracted text", {
                fileName,
            ...
            this.evictIfNeeded();
            logger_1.default.info("[PDF-PARSE] Successfully processed PDF", {
                fileName,
            ...
            logger_1.default.warn("[PDF-PARSE] Failed to process PDF", { error: error.message });
            return null;
        }
    }
    ...
    static extractAbstract(text) {
        const match = text.match(/abstract\s*[:\-—]?\s*([\s\S]*?)(?=\b(?:introduction|keywords|1\.\s))/i);
        if (match) {
    ...
    static findReferenceSection(text) {
        const patterns = [
    ...
    static splitReferenceEntries(refSection) {
    ...
    static parseSingleReference(text) {
        text = text.trim();
        if (text.length < 10)
            return null;
    ...
    static extractTitleFromRef(text) {
        ...
        if (ieeeMatch)
            return ieeeMatch[1].trim();
        const firstSentence = clean.match(/^([A-Z][^.]{10,200}\.)/);
        if (firstSentence)
            return firstSentence[1].replace(/\.$/, "").trim();
        return undefined;
    }
    static extractJournal(text) {
        const match = text.match(/(?:[A-Z][a-z]+ (?:Journal|Review|Research|Letters|Transactions|Proceedings|Annals|Archives|International Journal|British Journal|American Journal)[A-Za-z\s,.&]*)/);
...
exports.GrobidService = GrobidService;
