...
exports.HtmlExportService = void 0;
...
class HtmlExportService {
     ...
     */
    static async generateProjectHtml(project, // Using any for Project to avoid strict Prisma type issues in this snippet
    options) {
    ...
    <body>`;
        ...
        html += await this.convertTipTapToHtml(project.content, project.citations || [], options.citationStyle || "apa", options, state);
        html += `</div>`;
        ...
        if (options.resolvedCitations?.bibliography &&
            ...
            options.resolvedCitations.bibliography.forEach((entry) => {
    ...
    static generateCoverPage(project, options) {
    ...
    static async convertTipTapToHtml(content, citations = [], style = "apa", options, state = { citationNodeIndex: 0 }) {
    ...
    static extractTextHtml(node, citations = [], style = "apa", options, state = { citationNodeIndex: 0 }) {
        if (!node.content)
            return "";
        let html = "";
    ...
    static formatInTextCitation(citation, style) {
        ...
        const year = csl.issued?.["date-parts"]?.[0]?.[0] || citation.year || "n.d.";
        ...
        if (authors.length > 2) {
            ...
            const secondText = typeof secondAuthor === "string"
...
exports.HtmlExportService = HtmlExportService;
