...
exports.WorkspaceAnalyticsService = void 0;
...
class WorkspaceAnalyticsService {
     ...
     */
    static async getWorkspaceAnalytics(workspaceId) {
        try {
            // 1. Get task status distribution
            const statusCounts = await prisma_1.prisma.workspaceTask.groupBy({
                by: ["status"],
                where: { workspace_id: workspaceId },
                _count: { _all: true },
            });
            // 2. Get task priority distribution
            const priorityCounts = await prisma_1.prisma.workspaceTask.groupBy({
                by: ["priority"],
                where: { workspace_id: workspaceId },
                _count: { _all: true },
            });
            ...
            const allMembers = await prisma_1.prisma.workspaceMember.findMany({
                where: { workspace_id: workspaceId },
                include: {
                    user: {
            ...
            const allAssignments = await prisma_1.prisma.taskAssignee.findMany({
                where: {
            ...
            const memberActivity = Array.from(memberStatsMap.entries()).map(([name, stats]) => ({
                name,
                ...stats,
            }));
            ...
            const completedTasksHistory = await prisma_1.prisma.workspaceTask.findMany({
                where: {
            ...
            const overdueCount = await prisma_1.prisma.workspaceTask.count({
                where: {
                    workspace_id: workspaceId,
                    status: { not: "done" },
                    due_date: { lt: now },
            ...
            const upcomingCount = await prisma_1.prisma.workspaceTask.count({
                where: {
                    workspace_id: workspaceId,
                    status: { not: "done" },
                    due_date: {
            ...
            completedTasksHistory.forEach((task) => {
                const durationMs = task.updated_at.getTime() - task.created_at.getTime();
                totalCompletionTimeDays += durationMs / (1000 * 60 * 60 * 24);
            });
            const avgCompletionDays = completedTasksHistory.length > 0
                ? parseFloat((totalCompletionTimeDays / completedTasksHistory.length).toFixed(1))
                : 0;
            // 7. High-level stats
            const totalTasks = await prisma_1.prisma.workspaceTask.count({
                where: { workspace_id: workspaceId },
            });
            const totalMembers = await prisma_1.prisma.workspaceMember.count({
                where: { workspace_id: workspaceId },
            });
            const doneStatusCount = statusCounts.find((s) => s.status === "done");
    ...
    static async getProjectAnalytics(projectId) {
        try {
            ...
            const recentTasks = tasks.filter((t) => new Date(t.updated_at) >= sevenDaysAgo);
            return {
                projectId,
                ...
                recentActivity: {
                    ...
                    tasksCompleted: recentTasks.filter((t) => t.status === "done")
                        .length,
                },
            };
        }
        catch (error) {
            logger_1.default.error(`Error fetching project analytics for ${projectId}:`, error);
            throw error;
        }
    }
    /**
     ...
     */
    static async getWorkspaceAnalyticsWithProjects(workspaceId) {
        try {
            ...
            logger_1.default.error("Error fetching workspace analytics with projects:", error);
            throw error;
        }
    }
}
