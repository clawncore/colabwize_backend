...
const recycleBinService_1 = require("./recycleBinService");
const logger_1 = __importDefault(require("../monitoring/logger"));
const pdfConversionService_1 = require("./pdfConversionService");
const workspaceActivityService_1 = require("./workspaceActivityService");
const citationMappingService_1 = require("./citationMappingService");
class DocumentUploadService {
    /**
     * Creates a project with an uploaded document
     */
    static async createProjectWithDocument(userId, title, description, file, workspaceId, linkedLibrary) {
        ...
        let projectContent;
        try {
            const parsed = citationMappingService_1.CitationMappingService.parseDocument(extractedContent, format);
            ...
            if (parsed && parsed.content && parsed.content.length > 0) {
                ...
                const bibNodes = parsed.content.filter((n) => n.type === "bibliographyEntry");
            ...
            logger_1.default.warn("CitationMappingService failed, falling back to plain text:", {
        ...
        try {
            const bibNodes = projectContent?.content?.filter((n) => n.type === "bibliographyEntry") || [];
            if (bibNodes.length > 0) {
                const citationData = bibNodes.map((node) => {
                ...
                await prisma_1.prisma.citation.createMany({
                    data: citationData,
                    skipDuplicates: true,
                });
                logger_1.default.info(`Imported ${citationData.length} bibliography entries for project ${project.id}`);
            }
        }
        catch (citeError) {
            logger_1.default.warn("Failed to persist bibliography entries:", { error: citeError.message });
            // Non-fatal: project is still created
        }
        // Log workspace activity and notify members if applicable
        if (workspaceId) {
            await workspaceActivityService_1.WorkspaceActivityService.logActivity(workspaceId, userId, "PROJECT_CREATED", { title: project.title, projectId: project.id });
            try {
                const { createNotification } = require("./notificationService");
                const workspaceMembers = await prisma_1.prisma.workspaceMember.findMany({
                    where: { workspace_id: workspaceId, user_id: { not: userId } },
                });
                for (const member of workspaceMembers) {
                    await createNotification(member.user_id, "document_shared", "New Project Created", `A new project "${title}" has been created in your workspace.`, { workspaceId, documentId: project.id });
                }
            }
            catch (notifError) {
                logger_1.default.error("Failed to send project creation notification:", notifError);
            }
        }
    ...
    static async getUserProjects(userId, options) {
        ...
        return projects.map((project) => this.mapProjectWithProgress(project));
    }
    /**
     ...
     */
    static async checkProjectExists(projectId) {
     ...
     * Updates a project
     */
    static async updateProject(projectId, userId, title, description, content, wordCount, citationStyle, outline, updates) {
    ...
    static async deleteProject(projectId, userId) {
        ...
        if (project.user_id !== userId) {
            logger_1.default.warn(`[DEBUG] Delete access denied. Owner: ${project.user_id}, Requestor: ${userId}`);
            throw new Error(`Access denied: Project owned by ${project.user_id}, not ${userId}`);
        }
        ...
        if (fullProject) {
            // Add to recycle bin
            await recycleBinService_1.RecycleBinService.addItemToRecycleBin(userId, "project", {
                id: fullProject.id,
     ...
     */
    static async createProject(userId, title, description, content, outline = null, workspaceId, linkedLibrary) {
        // Create project record in the database
        const project = await prisma_1.prisma.project.create({
            data: {
                user_id: userId,
    ...
    static async extractTextFromDocument(file) {
        ...
        try {
                ...
                case "pdf":
                    if (process.env.MATHPIX_APP_ID && process.env.MATHPIX_APP_KEY) {
                        try {
                            ...
                            logger_1.default.info("[PDF-CONVERSION] Attempting Mathpix conversion", {
                                filePath,
                            });
                            const html = await MathpixService.convertPdfToHtml(filePath);
                            logger_1.default.info("[PDF-CONVERSION] Mathpix conversion successful");
                            return { content: html, format: "html" };
                        }
                        catch (mathpixError) {
                            logger_1.default.warn("[PDF-CONVERSION] Mathpix conversion failed, falling back to local tools", {
                                error: mathpixError.message,
                            });
                        }
                    }
                    try {
                        logger_1.default.info("[PDF-CONVERSION] Attempting PDF to DOCX conversion", {
                            filePath,
                        });
                        const docxPath = await pdfConversionService_1.PdfConversionService.convertPdfToDocx(filePath);
                        logger_1.default.info("[PDF-CONVERSION] PDF to DOCX conversion successful, extracting content", { docxPath });
                        const html = await this.extractHtmlFromDOCX(docxPath);
                        try {
                            await promises_1.default.unlink(docxPath);
                            logger_1.default.info("[PDF-CONVERSION] Temporary DOCX file cleaned up", {
                                docxPath,
                            });
                        }
                        catch (cleanupError) {
                            logger_1.default.warn("[PDF-CONVERSION] Failed to clean up temporary DOCX file", {
                                docxPath,
                        ...
                        logger_1.default.warn("[PDF-CONVERSION] PDF to DOCX conversion failed, falling back to text extraction", {
            ...
            logger_1.default.error("Error extracting text from document", {
                error: error.message,
                stack: error.stack,
    ...
    static async extractTextFromPDF(filePath) {
        ...
        try {
            logger_1.default.info(`[PDF] Starting PDF parsing`, { filePath });
            const buffer = await promises_1.default.readFile(filePath);
            const data = await (0, pdf_parse_1.default)(buffer);
            const duration = Date.now() - startTime;
            logger_1.default.info(`[PERF] PDF Parsing Complete`, {
                duration,
                filePath,
     ...
     */
    static async extractHtmlFromDOCX(filePath) {
        try {
            const result = await mammoth_1.default.convertToHtml({ path: filePath });
            return result.value;
        }
        catch (error) {
            logger_1.default.error("Error parsing DOCX to HTML", { error, filePath });
            throw error;
        }
    }
    /**
     ...
     */
    static calculateProjectProgress(status) {
        switch (status) {
            case "completed":
                return 100;
            case "in-progress":
     ...
     */
    static mapProjectWithProgress(project) {
        if (!project)
            return null;
        return {
            ...project,
