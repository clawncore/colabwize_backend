...
class ContactService {
    ...
    static async generateTicketNumber() {
        ...
        const count = await prisma_1.prisma.contactRequest.count({
            where: {
    ...
    static async handleContactSubmission(data) {
        try {
            ...
            const contactRequest = await prisma_1.prisma.contactRequest.create({
                data: {
            ...
            const adminEmail = await secrets_service_1.SecretsService.getContactAdminEmail();
            try {
                await this.sendAdminNotification(adminEmail, data);
            }
            catch (emailError) {
                console.warn("Warning: Failed to send admin notification via Discord webhook:", emailError);
            ...
            try {
                ...
                console.warn("Warning: Failed to send admin email notification:", emailError);
            }
            ...
            try {
                ...
                console.warn("Warning: Failed to send user confirmation email:", emailError);
                // Continue with the process even if email fails
            }
            return {
                success: true,
    ...
    static async sendAdminNotification(adminEmail, // This parameter is kept for backward compatibility but not used
    data) {
        try {
            // Determine type of request
            const lowerSubject = data.subject.toLowerCase();
            const isFeatureRequest = lowerSubject.includes("feature") || lowerSubject === "feature request";
            const isDemoRequest = lowerSubject.includes("demo") || lowerSubject === "schedule demo";
            // Select appropriate webhook URL from environment variables
            let webhookUrl = await secrets_service_1.SecretsService.getContactWebhookUrl();
            if (isFeatureRequest) {
                webhookUrl = await secrets_service_1.SecretsService.getFeatureWebhookUrl();
            }
            else if (isDemoRequest) {
                webhookUrl = await secrets_service_1.SecretsService.getDemoWebhookUrl();
            }
            if (!webhookUrl) {
                console.warn("Warning: Discord webhook URL not configured in environment variables for this request type");
                return false;
            }
            ...
            if (data.ip_address || data.user_agent) {
                ...
                if (data.user_agent) {
                    metadataFields.push({
                        name: "🖥️ User Agent",
                        value: data.user_agent.length > 1024
            ...
            console.error("Error sending admin notification via Discord webhook:", error);
            return false;
        }
    }
    ...
    static async sendAdminEmail(data) {
        try {
            const adminEmail = await secrets_service_1.SecretsService.getContactAdminEmail();
            if (!adminEmail) {
    ...
    static async getContactRequests(status, limit = 50) {
        try {
            const whereClause = status ? { status } : {};
            const contactRequests = await prisma_1.prisma.contactRequest.findMany({
                where: whereClause,
                orderBy: {
                    created_at: "desc",
                },
    ...
    static async updateContactRequestStatus(id, status) {
        try {
            const updatedRequest = await prisma_1.prisma.contactRequest.update({
                where: {
                    id: id,
                },
                data: {
    ...
    static async getContactRequestById(id) {
        try {
            const contactRequest = await prisma_1.prisma.contactRequest.findUnique({
                where: {
                    id: id,
                },
            });
