...
exports.CitationConfidenceService = void 0;
...
class CitationConfidenceService {
     ...
     */
    static calculateRecencyScore(year, field = "default") {
        const currentYear = new Date().getFullYear();
        const age = currentYear - year;
        const threshold = this.FIELD_THRESHOLDS[field.toLowerCase()] ||
        ...
        if (age <= threshold / 3) {
            ...
            return {
                ...
                status: "outdated",
                yearsOld: age,
                recommendation: "Historical source - ensure recent findings are also included",
            };
        }
    }
    /**
     ...
     */
    static calculateConfidenceScore(citations, textLength, field = "default") {
        ...
        const recencyScores = citations.map((c) => this.calculateRecencyScore(c.year, field).score);
        ...
        const coverageScore = Math.min(100, Math.max(0, 100 - (wordsPerCitation - 200) / 5));
        // 3. Quality Score (20% weight) - based on citation counts
        const validCitations = citations.filter((c) => c.citationCount !== undefined && c.citationCount !== null);
        const avgCitationCount = validCitations.length > 0
            ? validCitations.reduce((sum, c) => sum + (c.citationCount || 0), 0) /
                validCitations.length
            : 0;
        const qualityScore = Math.min(100, avgCitationCount > 0 ? Math.log10(avgCitationCount + 1) * 30 : 50);
        // 4. Diversity Score (10% weight) - variety of sources
        const uniqueAuthors = new Set(citations.map((c) => c.author)).size;
        const diversityScore = Math.min(100, (uniqueAuthors / citations.length) * 100);
        // Calculate weighted overall score
        const overall = Math.round(avgRecency * 0.4 +
            coverageScore * 0.3 +
            qualityScore * 0.2 +
            diversityScore * 0.1);
        // Determine status
        ...
        if (unreliable.length > 0) {
            warnings.push(`Detected ${unreliable.length} citations that could not be verified in public databases.`);
            suggestions.push("Verify the authenticity of your sources using valid databases.");
     ...
     */
    static async analyzeProjectCitations(projectId, userId, field = "default") {
        ...
        try {
            ...
            const citationData = citations.map((c) => {
                ...
                const currentYear = new Date().getFullYear();
                if (year > currentYear) {
                    logger_1.default.warn("Future year detected in citation - marking as invalid", {
                        citationId: c.id,
                        year,
                        currentYear,
                    });
            ...
            const overallConfidence = this.calculateConfidenceScore(citationData, textLength, field);
            // Calculate breakdown
            const breakdown = {
                recent: 0,
                acceptable: 0,
            ...
            try {
                ...
                // I need the name now.
                const projectDetails = await prisma_1.prisma.project.findUnique({
                    where: { id: projectId },
                });
                if (user && user.email && projectDetails) {
                    await emailService_1.EmailService.sendScanCompletionEmail(user.email, user.full_name || "ColabWize User", "citations", projectDetails.title || "Untitled Project", `Confidence Score: ${Math.round(overallConfidence.overall)}/100\nTotal Citations: ${citations.length}\nStatus: ${overallConfidence.status.toUpperCase()}`, `${await secrets_service_1.SecretsService.getFrontendUrl()}/dashboard/editor/${projectId}?tab=citations`);
                }
            }
            catch (emailError) {
                logger_1.default.error("Failed to send citation analysis completion email", {
            ...
            if (!(error instanceof BillingGateway_1.BillingError)) {
                logger_1.default.error("Error analyzing project citations", {
                    error: error.message,
                    projectId,
                });
            }
     ...
     */
    static async addCitation(projectId, userId, citationData) {
        try {
            ...
            const existing = await prisma_1.prisma.citation.findFirst({
                where: {
                    project_id: projectId,
            ...
            if (!formatValidation.valid) {
                logger_1.default.warn("Citation format validation failed", {
            ...
            if (citationData.doi) {
                vault_verified = await this.verifyCitationWithCrossRef(citationData.doi, citationData.title);
                if (!vault_verified) {
                    logger_1.default.warn("Citation verification failed", {
            ...
            if (citationData.year > currentYear) {
                logger_1.default.warn("Future year in citation - adjusting to current year", {
            ...
            // Create citation
            const citation = await prisma_1.prisma.citation.create({
                data: {
                    project: {
            ...
            logger_1.default.info("Citation added to project", {
     ...
     */
    static scanContentForCitations(content) {
        ...
        for (const sentence of sentences) {
            ...
            const hasCitation = /\([A-Za-z\s.,'\-]+,?\s?\d{4}\)|\[\d+\]/.test(trimmed) ||
            ...
            if (/\d+%|\d+ percent|data shows|study found/.test(trimmed)) {
                results.push({
                    sentence: trimmed,
                    suggestion: "This sentence resembles a statistical claim commonly supported by citations in academic writing.",
                    type: "statistic",
                });
                continue;
            }
            // Check for Definitions
            if (/is defined as|refers to|can be described as|is known as/.test(trimmed)) {
                results.push({
                    sentence: trimmed,
                    suggestion: "This sentence resembles a definitional claim commonly supported by citations in academic writing.",
                    type: "definition",
                });
                continue;
            }
            // Check for Factual Claims (strong assertations)
            if (/evidence suggests|research indicates|it is well known|studies have shown|historically/.test(trimmed)) {
                results.push({
                    sentence: trimmed,
                    suggestion: "This sentence resembles a strong assertion commonly supported by citations in academic writing.",
                    type: "factual_claim",
                });
                continue;
            }
    ...
    static validateCitationFormat(citation) {
        ...
        if (!citation.year ||
            ...
            issues.push(`Invalid year: ${citation.year} (must be between 1900 and ${currentYear + 1})`);
        }
        // Journal articles should ideally have a DOI
        if (citation.type === "journal-article" && !citation.doi) {
            issues.push("Journal article missing DOI (consider adding for verification)");
        }
        return {
     ...
     */
    static async verifyCitationWithCrossRef(doi, title) {
        try {
            // Query CrossRef API
            ...
            if (response.status === 200 && response.data?.message) {
                ...
                if (work.DOI && work.DOI.toLowerCase() === doi.toLowerCase()) {
                    ...
                    if (title && work.title?.[0]) {
                        ...
                        if (!normalizedWorkTitle.includes(normalizedTitle.substring(0, 20))) {
                            logger_1.default.warn("Title mismatch in CrossRef verification", {
                                provided: title,
                    ...
                    logger_1.default.info("Citation verified with CrossRef", { doi });
                    return true;
                }
            }
            logger_1.default.warn("Citation not found in CrossRef", { doi });
            return false;
        }
        catch (error) {
            if (error.response?.status === 404) {
                logger_1.default.warn("Citation DOI not found in CrossRef", { doi });
                return false;
            }
            logger_1.default.error("CrossRef verification failed", {
                error: error.message,
                doi,
            });
     ...
     */
    static async verifyCitationWithPubMed(title) {
        try {
            if (!title || title.length < 10)
                return false;
            // Use E-Utilities ESearch
            const response = await axios_1.default.get("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi", {
                params: {
                    db: "pubmed",
                    term: `${title}[Title]`,
                    retmode: "json",
            ...
            if (response.status === 200 && response.data?.esearchresult) {
                ...
                if (count > 0) {
                    logger_1.default.info("Citation verified with PubMed", { title });
                    return true;
                }
            }
            return false;
            ...
            logger_1.default.warn("PubMed verification failed", { error: error.message, title });
     ...
     */
    static async verifyCitationWithArxiv(title) {
        try {
            if (!title || title.length < 10)
                return false;
            // Use arXiv API
            ...
            logger_1.default.warn("arXiv verification failed", { error: error.message, title });
            return false;
        }
    }
    /**
     ...
     */
    static async verifyCitationWithOpenAlex(title) {
        try {
            if (!title || title.length < 10)
                return false;
            ...
            if (response.status === 200 && response.data?.results?.length > 0) {
                ...
                if (work.title && work.title.toLowerCase().includes(title.toLowerCase().substring(0, 20))) {
                    logger_1.default.info("Citation verified with OpenAlex", { title });
                    return true;
                }
            }
            return false;
            ...
            logger_1.default.warn("OpenAlex verification failed", { error: error.message, title });
    ...
    static async verifySingleCitation(citation) {
        ...
        if (citation.doi) {
            ...
            if (crossRefValid)
                return { vault_verified: true, source: 'crossref' };
        }
     ...
     */
    static async findCitationMetadata(query) {
        try {
            ...
            const openAlexResponse = await axios_1.default.get("https://api.openalex.org/works", {
            ...
            logger_1.default.warn("Auto-Fixer search failed", { error: e });
            return null;
        }
    }
}
exports.CitationConfidenceService = CitationConfidenceService;
