...
class ContentNormalizer {
     ...
     */
    static async normalizeContent(content) {
    ...
    static async normalizeImageNode(node) {
        ...
        if (!isDataUrl) {
            if (!isHttp) {
                ...
                try {
                    ...
                    const signedUrl = await supabaseStorageService_1.SupabaseStorageService.createSignedUrl(cleanPath, 3600);
                    if (signedUrl) {
                        node.attrs.src = signedUrl;
                        // Add a flag so export service knows it's resolved
                        node.attrs.resolved = true;
                        logger_1.default.debug("Resolved signed URL for image", { original: src });
                    }
                }
                catch (error) {
                    logger_1.default.warn("Failed to sign image URL", { src, error });
                }
            }
        }
    }
    static normalizeTableNode(node) {
