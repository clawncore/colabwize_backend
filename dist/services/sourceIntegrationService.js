...
exports.SourceIntegrationService = void 0;
...
class SourceIntegrationService {
     ...
     */
    static async trackSourceInteraction(data) {
        try {
            ...
            const existing = await prisma_1.prisma.sourceInteraction.findUnique({
                where: {
            ...
            if (existing) {
                ...
                await prisma_1.prisma.sourceInteraction.update({
                ...
                await prisma_1.prisma.sourceInteraction.create({
                    data: {
                        project_id: projectId,
                        user_id: userId,
                        source_id: sourceId,
            ...
            logger_1.default.info("Source interaction tracked", { sourceId, projectId, userId });
        }
        catch (error) {
            logger_1.default.error("Error tracking source interaction", { error: error.message });
            throw error;
        }
    }
    /**
     ...
     */
    static async markCitationAdded(projectId, userId, sourceId) {
        try {
            const interaction = await prisma_1.prisma.sourceInteraction.findUnique({
                where: {
                    project_id_user_id_source_id: {
                        project_id: projectId,
                        user_id: userId,
            ...
            logger_1.default.info("Citation marked for source", { sourceId, projectId, userId });
        }
        catch (error) {
            logger_1.default.error("Error marking citation", { error: error.message });
            throw error;
        }
    }
    /**
     ...
     */
    static async verifySourceIntegration(projectId, userId) {
        try {
            const interactions = await prisma_1.prisma.sourceInteraction.findMany({
                where: {
                    project_id: projectId,
                    user_id: userId,
                },
            ...
            const sourcesWithAdequateReading = interactions.filter((i) => i.total_reading_time_ms >= 30000).length;
            const sourcesWithProperTiming = interactions.filter((i) => i.citation_preceded_by_reading).length;
            ...
            logger_1.default.error("Error verifying source integration", { error: error.message });
            throw error;
        }
    }
    /**
     ...
     */
    static async getSourceAnalytics(projectId, userId) {
        try {
            return await prisma_1.prisma.sourceInteraction.findMany({
                where: {
                    project_id: projectId,
                    user_id: userId,
                },
            ...
            logger_1.default.error("Error getting source analytics", { error: error.message });
            throw error;
        }
    }
}
exports.SourceIntegrationService = SourceIntegrationService;
