    ...
    static async recordLogin(userId, ipAddress, userAgent) {
        try {
            ...
            if (formattedIp === "127.0.0.1" || formattedIp === "::1") {
                ...
                if (publicIp)
                    formattedIp = publicIp;
            }
            const location = await (0, ipGeolocation_1.getLocationFromIp)(formattedIp);
            ...
            await securityService_1.SecurityService.recordLoginAttempt(userId, formattedIp, userAgent, location || "Unknown", "success");
        }
        catch (error) {
            logger_1.default.error("Failed to record login", { error: error.message, userId });
        }
    }
}
