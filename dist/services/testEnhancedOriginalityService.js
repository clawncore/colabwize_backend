...
const enhancedOriginalityDetectionService_1 = require("./enhancedOriginalityDetectionService");
...
async function runEnhancedOriginalityTests() {
    ...
    try {
        ...
        const result1 = await enhancedOriginalityDetectionService_1.EnhancedOriginalityDetectionService.calculateEnhancedSimilarity(testDocuments.academicPaper, testDocuments.paraphrasedContent);
        console.log(`   Similarity Score: ${(result1 * 100).toFixed(2)}%`);
        console.log(`   Classification: ${result1 > 0.6 ? "High Similarity Detected" : "Acceptable Originality"}`);
    }
    catch (error) {
    ...
    try {
        const result2 = await enhancedOriginalityDetectionService_1.EnhancedOriginalityDetectionService.calculateEnhancedSimilarity(testDocuments.commonPhrases, "In conclusion, the findings show important results. On the other hand, some issues remain. For example, collection had problems. In other words, the approach needed changes. As a result, future studies should consider these elements.");
    ...
    try {
        const ngramScore = enhancedOriginalityDetectionService_1.EnhancedOriginalityDetectionService["calculateNGramSimilarity"]("Machine learning algorithms improve processing", "Deep learning models enhance computation");
        console.log(`   N-gram Similarity Score: ${(ngramScore * 100).toFixed(2)}%`);
    ...
    try {
        const jaccardScore = enhancedOriginalityDetectionService_1.EnhancedOriginalityDetectionService["calculateJaccardSimilarity"]("natural language processing artificial intelligence", "artificial intelligence machine learning");
        console.log(`   Jaccard Similarity Score: ${(jaccardScore * 100).toFixed(2)}%`);
    ...
    try {
        const isAcademic1 = enhancedOriginalityDetectionService_1.EnhancedOriginalityDetectionService["isAcademicLanguage"]("Furthermore, the evidence suggests significant implications for future research.");
        const isAcademic2 = enhancedOriginalityDetectionService_1.EnhancedOriginalityDetectionService["isAcademicLanguage"]("The cat sat on the mat and looked around.");
    ...
    try {
        const hasPassive1 = enhancedOriginalityDetectionService_1.EnhancedOriginalityDetectionService["hasPassiveVoice"]("The experiment was conducted by the researchers.");
        const hasPassive2 = enhancedOriginalityDetectionService_1.EnhancedOriginalityDetectionService["hasPassiveVoice"]("Researchers conducted the experiment.");
    ...
    try {
        const classification = enhancedOriginalityDetectionService_1.EnhancedOriginalityDetectionService["classifyMatch"](65, // similarity score
        "According to recent studies, the methodology shows significant improvements.", "academic");
    ...
    console.log("   • Multi-database academic source integration (CrossRef, Semantic Scholar, arXiv, IEEE, PubMed)");
    console.log("   • Advanced NLP with multiple similarity algorithms (semantic, n-gram, Jaccard)");
    console.log("   • Context-aware classification with linguistic feature analysis");
