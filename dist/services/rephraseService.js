...
exports.RephraseService = void 0;
const prisma_1 = require("../lib/prisma");
const logger_1 = __importDefault(require("../monitoring/logger"));
const openaiService_1 = require("./openaiService");
const string_similarity_1 = require("string-similarity");
const AbuseGuard_1 = require("./AbuseGuard");
const usageService_1 = require("./usageService");
class RephraseService {
    /**
     * Generate rephrase suggestions for flagged text
     */
    static async generateRephraseSuggestions(scanId, matchId, originalText, userId, mode = AbuseGuard_1.RephraseMode.ACADEMIC) {
        try {
            logger_1.default.info("Generating rephrase suggestions", { scanId, matchId, mode });
            // 1. Abuse & Similarity Guard
            const abuseCheck = await AbuseGuard_1.AbuseGuard.checkAbuse(userId, originalText);
            // Degrade mode if abuse detected
            let effectiveMode = mode;
            if (abuseCheck.isAbuse) {
                if (abuseCheck.degradeTo === "CACHED") {
                    logger_1.default.warn(`Abuse limit hit: Forcing CACHED response for user ${userId}`);
                    return await this.getCachedOrFallback(scanId, originalText);
                }
                if (abuseCheck.degradeTo === "LOCAL") {
                    logger_1.default.warn(`Abuse limit hit: Forcing LOCAL response for user ${userId}`);
                    effectiveMode = AbuseGuard_1.RephraseMode.QUICK; // Limit cost/usage
                }
            }
            ...
            const unitsConsumed = AbuseGuard_1.AbuseGuard.calculateCost(originalText, effectiveMode);
            ...
            usageService_1.UsageService.trackUsage(userId, 'rephrase_requests').catch((e) => logger_1.default.error("Failed to track usage", e));
            ...
            if (!isTemporary) {
                ...
                if (effectiveMode !== AbuseGuard_1.RephraseMode.DEEP) {
                    const existingSuggestions = await prisma_1.prisma.rephraseSuggestion.findMany({
                        where: {
                            scan_id: scanId,
                    ...
                    if (existingSuggestions.length > 0) {
                        logger_1.default.info("Found cached rephrase suggestions");
            ...
            if (!isTemporary) {
                for (const suggestion of suggestions) {
                    const saved = await prisma_1.prisma.rephraseSuggestion.create({
                        data: {
                            scan_id: scanId,
                            match_id: matchId,
                            original_text: originalText,
            ...
            logger_1.default.info(`Generated ${savedSuggestions.length} rephrase suggestions`);
     ...
     */
    static async getCachedOrFallback(scanId, originalText) {
        // Try DB Cache
        const existingSuggestions = await prisma_1.prisma.rephraseSuggestion.findMany({
     ...
     */
    static async generateAIRephrases(originalText, mode) {
        try {
            // 1. QUICK / LOCAL Mode
            if (mode === AbuseGuard_1.RephraseMode.QUICK) {
            ...
            const prompt = mode === AbuseGuard_1.RephraseMode.DEEP
            ...
            const response = await openaiService_1.OpenAIService.generateCompletion(prompt, {
                maxTokens: mode === AbuseGuard_1.RephraseMode.DEEP ? 500 : 300,
                temperature: mode === AbuseGuard_1.RephraseMode.DEEP ? 0.8 : 0.7,
                model: mode === AbuseGuard_1.RephraseMode.DEEP ? "gpt-4" : "gpt-3.5-turbo" // Hypothetical model switch
            });
            ...
            logger_1.default.error("Error generating AI rephrases", { error: error.message });
            // Fallback
            return this.generateLocalRephrases(originalText);
        }
    }
     ...
     */
    static generateLocalRephrases(originalText) {
        const suggestions = [];
        ...
        if (restructuredSuggestion !== originalText &&
        ...
        if (voiceTransformedSuggestion !== originalText &&
        ...
        if (paraphrasedSuggestion !== originalText &&
        ...
        if (academicSuggestion !== originalText &&
     ...
     */
    static isTooSimilar(suggestions, newSuggestion) {
        for (const suggestion of suggestions) {
            const similarity = (0, string_similarity_1.compareTwoStrings)(suggestion, newSuggestion);
            if (similarity > 0.8) {
                return true;
            }
        }
     ...
     */
    static restructureSentence(text) {
     ...
     */
    static adjustAcademicTone(text) {
     ...
     * Parse AI response
     */
    static parseSuggestions(response) {
     ...
     */
    static async getScanSuggestions(scanId, userId) {
        try {
            // Verify scan belongs to user
            const scan = await prisma_1.prisma.originalityScan.findFirst({
                where: {
            ...
            const suggestions = await prisma_1.prisma.rephraseSuggestion.findMany({
                where: {
                    scan_id: scanId,
                },
                orderBy: {
            ...
            logger_1.default.error("Error getting scan suggestions", {
                error: error.message,
                scanId,
            });
...
exports.RephraseService = RephraseService;
