...
exports.broadcastWorkspaceNotification = broadcastWorkspaceNotification;
exports.getUserNotifications = getUserNotifications;
exports.markNotificationAsRead = markNotificationAsRead;
exports.markAllNotificationsAsRead = markAllNotificationsAsRead;
exports.deleteNotification = deleteNotification;
exports.dismissNotification = dismissNotification;
exports.snoozeNotification = snoozeNotification;
...
exports.sendSubscriptionCreatedNotification = sendSubscriptionCreatedNotification;
exports.sendSubscriptionUpdatedNotification = sendSubscriptionUpdatedNotification;
exports.sendSubscriptionCancelledNotification = sendSubscriptionCancelledNotification;
exports.sendSubscriptionRenewedNotification = sendSubscriptionRenewedNotification;
exports.sendSubscriptionExpiringNotification = sendSubscriptionExpiringNotification;
...
exports.sendPaymentRefundedNotification = sendPaymentRefundedNotification;
exports.sendInvoiceAvailableNotification = sendInvoiceAvailableNotification;
exports.broadcastGenericNotification = broadcastGenericNotification;
...
const notificationSettingsService_1 = require("./notificationSettingsService");
// Mapping of notification types to subscription features
const notificationTypeToSubscriptionFeature = {
...
async function canUserReceiveNotification(userId, type) {
    try {
        ...
        const featureMap = {
            ...
            collaboration_request_collaborator_request: "collaboration.requestCollaborator",
...
async function sendAINotification(userId, type, title, message, data) {
    try {
...
async function createNotification(userId, type, title, message, data) {
    try {
        ...
        if (!canReceive) {
            console.log(`User ${userId} cannot receive ${type} notification due to subscription limitations`);
            return null;
        }
        ...
        if (settings.quiet_hours_enabled) {
            ...
            if (isTimeInRange(currentTime, settings.quiet_hours_start_time, settings.quiet_hours_end_time)) {
...
async function broadcastWorkspaceNotification(workspaceId, actorId, type, messageGenerator, data, recipientIds = []) {
    try {
        ...
        if (recipientIds.length > 0) {
            const recipient = members.find((m) => m.user_id === recipientIds[0]);
            if (recipient)
        ...
        for (const member of members) {
            ...
            if (userId === actorId) {
                ...
                notificationContent = messageGenerator.others(actorName, primaryRecipientName);
            }
            await createNotification(userId, type, notificationContent.title, notificationContent.message, data);
        }
    }
    catch (error) {
...
async function getUserNotifications(userId, limit = 20, offset = 0, filters) {
    try {
        ...
        console.log("Fetching notifications with where clause:", JSON.stringify(where, null, 2));
...
async function markAllNotificationsAsRead(userId) {
    try {
        const notifications = await prisma_1.prisma.notification.updateMany({
...
// Snooze notification
async function snoozeNotification(notificationId, snoozeUntil) {
    try {
        const notification = await prisma_1.prisma.notification.update({
            where: { id: notificationId },
...
// Get unread notification count for a user
async function getUnreadNotificationCount(userId, filters) {
    try {
        const where = { user_id: userId, read: false, dismissed: false };
        if (filters?.workspaceId) {
            where.data = {
                path: ["workspaceId"],
                equals: filters.workspaceId,
...
function shouldSendNotification(settings, type) {
    ...
    const categoryPrefix = Object.keys(categoryMap).find((prefix) => settingField.startsWith(prefix));
    if (categoryPrefix) {
...
async function sendEmailNotification(userId, type, title, message, data) {
    try {
        ...
        // Send email
        await emailService_1.EmailService.sendNotificationEmail(user.email, user.full_name || "", title, message, type);
        console.log(`Email notification sent to user ${userId}: ${title} - ${message}`);
    }
    catch (error) {
...
async function sendSMSNotification(userId, type, title, message, data) {
    try {
...
async function sendPushNotification(userId, type, title, message, data) {
    try {
        ...
        await pushNotificationService_1.PushNotificationService.sendToUser(userId, title, message, stringData);
    }
    catch (error) {
...
async function getUserNotificationSettings(userId) {
    return await notificationSettingsService_1.NotificationSettingsService.getUserNotificationSettings(userId);
}
// Update user notification settings
async function updateUserNotificationSettings(userId, settingsData) {
    return await notificationSettingsService_1.NotificationSettingsService.updateUserNotificationSettings(userId, settingsData);
}
// Create billing notification
async function createBillingNotification(userId, type, title, message, data) {
    try {
...
async function sendSubscriptionCreatedNotification(userId, planName, amount, billingPeriod) {
    ...
    return await createBillingNotification(userId, "subscription_created", title, message, { planName, amount, billingPeriod });
}
// Send subscription updated notification
async function sendSubscriptionUpdatedNotification(userId, oldPlanName, newPlanName, amount) {
    ...
    return await createBillingNotification(userId, "subscription_updated", title, message, { oldPlanName, newPlanName, amount });
}
// Send subscription cancelled notification
async function sendSubscriptionCancelledNotification(userId, planName, endDate) {
    ...
    return await createBillingNotification(userId, "subscription_cancelled", title, message, { planName, endDate });
}
// Send subscription renewed notification
async function sendSubscriptionRenewedNotification(userId, planName, amount, nextBillingDate) {
    ...
    return await createBillingNotification(userId, "subscription_renewed", title, message, { planName, amount, nextBillingDate });
}
// Send subscription expiring notification
async function sendSubscriptionExpiringNotification(userId, planName, expirationDate, amount) {
    ...
    return await createBillingNotification(userId, "subscription_expiring", title, message, { planName, expirationDate, amount });
}
// Send payment success notification
async function sendPaymentSuccessNotification(userId, amount, planName, transactionId) {
    ...
    return await createBillingNotification(userId, "payment_success", title, message, { amount, planName, transactionId });
}
// Send payment failed notification
async function sendPaymentFailedNotification(userId, amount, planName, errorMessage) {
    ...
    return await createBillingNotification(userId, "payment_failed", title, message, { amount, planName, errorMessage });
}
// Send payment refunded notification
async function sendPaymentRefundedNotification(userId, amount, planName, transactionId) {
    ...
    return await createBillingNotification(userId, "payment_refunded", title, message, { amount, planName, transactionId });
}
// Send invoice available notification
async function sendInvoiceAvailableNotification(userId, invoiceId, amount, dueDate, downloadUrl) {
    ...
    return await createBillingNotification(userId, "invoice_available", title, message, { invoiceId, amount, dueDate, downloadUrl });
}
// Broadcast a generic notification to all workspace members (except the actor)
async function broadcastGenericNotification(workspaceId, actorId, type, title, message, data) {
    try {
        // 1. Get workspace members
        const members = await prisma_1.prisma.workspaceMember.findMany({
            where: { workspace_id: workspaceId },
        ...
        const notificationPromises = members
            ...
            .map((member) => createNotification(member.user_id, type, title, message, data));
