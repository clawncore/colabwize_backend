...
class SubscriptionService {
     ...
     */
    static async getActivePlan(userId, existingSubscription) {
        ...
        if (!["active", "trialing", "on_trial", "past_due", "cancelled"].includes(subscription.status)) {
            return "free";
        }
        return subscription.plan;
    }
    ...
    static getPlanLimits(plan) {
        ...
        const resolvedLimits = exports.plans[normalizedPlan] || exports.plans.free;
        console.log(`[LIMITS_RESOLUTION] Plan: "${plan}" (Normalized: "${normalizedPlan}") -> Using limits for key: ${resolvedLimits === exports.plans.free ? "free (fallback)" : normalizedPlan}`);
        return resolvedLimits;
    }
    /**
     * Check feature access
     */
    static async checkFeatureAccess(userId, feature, existingSubscription) {
     ...
     */
    static async checkMonthlyUsage(userId, feature) {
        const now = new Date();
        ...
        try {
            ...
            logger_1.default.warn("Failed to fetch subscription for usage check, defaulting to calendar month", { userId });
        }
        // Map feature to limit key to ensure consistency with PLAN_LIMITS
        let limitKey = feature;
        if (feature === "scan")
            limitKey = "scans_per_month";
        if (feature === "citation_check")
            limitKey = "citation_audit";
        if (feature === "rephrase")
            limitKey = "rephrase_suggestions";
    ...
    static async resetMonthlyUsage() {
        ...
        await prisma_1.prisma.usageTracking.deleteMany({
            where: {
                period_end: { lt: lastMonth },
            },
        });
        logger_1.default.info("Monthly usage reset completed");
    }
    /**
     ...
     */
    static async upsertSubscription(userId, data) {
        ...
        EntitlementService_1.EntitlementService.rebuildEntitlements(userId).catch((err) => {
            logger_1.default.error("Failed to rebuild entitlements async", {
                userId,
                error: err.message,
            });
        });
        logger_1.default.info("Subscription upserted", {
            userId,
            plan: data.plan,
            status: data.status,
        });
    ...
    static async cancelSubscription(userId) {
        ...
        await lemonSqueezyService_1.LemonSqueezyService.cancelSubscription(subscription.lemonsqueezy_subscription_id);
        // Update in database
        ...
        logger_1.default.info("Subscription canceled", { userId });
    ...
    static async reactivateSubscription(userId) {
        ...
        logger_1.default.info("Subscription reactivated", { userId });
        await EntitlementService_1.EntitlementService.rebuildEntitlements(userId);
        return true;
    }
    /**
    ...
    static async ensureLemonCustomer(user) {
        try {
            ...
            logger_1.default.info("Checking for existing Lemon Squeezy customer by email", {
                email: user.email,
            });
            const existingCustomers = await lemonSqueezyService_1.LemonSqueezyService.getCustomersByEmail(user.email);
            let customerId;
            if (existingCustomers && existingCustomers.length > 0) {
                customerId = existingCustomers[0].id;
                logger_1.default.info("Found existing Lemon Squeezy customer", {
                    email: user.email,
                    customerId,
                });
            }
                ...
                logger_1.default.info("Initializing new Lemon Squeezy customer for user", {
                    userId: user.id,
                });
                const newCustomer = await lemonSqueezyService_1.LemonSqueezyService.createCustomer(user.email, user.name || "Customer");
            ...
            logger_1.default.info("Linked Lemon Squeezy customer successfully", {
                userId: user.id,
                customerId,
            });
            return customerId;
            ...
            logger_1.default.error("Failed to ensure Lemon Squeezy customer:", error);
     ...
     */
    static async updateAutoUseCredits(userId, enabled) {
