...
exports.AuthorshipCertificateGenerator = void 0;
const logger_1 = __importDefault(require("../monitoring/logger"));
const authorshipReportService_1 = require("./authorshipReportService");
const qrcode_1 = __importDefault(require("qrcode"));
const node_fs_1 = __importDefault(require("node:fs"));
const node_path_1 = __importDefault(require("node:path"));
class AuthorshipCertificateGenerator {
    /**
     * Generate authorship certificate PDF with elegant landscape design
     */
    static async generateCertificate(options) {
        try {
            logger_1.default.info("Generating elegant authorship certificate", {
                projectId: options.projectId,
                userId: options.userId,
                type: options.certificateType || "authorship",
            });
            // Get authorship statistics
            const stats = await authorshipReportService_1.AuthorshipReportService.generateAuthorshipReport(options.projectId, options.userId);
            // Generate QR code for bottom-right corner
            const qrCodeDataUrl = options.includeQRCode && options.verificationUrl
                ? await this.generateQRCodeDataURL(options.verificationUrl)
                : null;
            // Generate certificate HTML
            const html = await this.generateCertificateHTML(options, stats, qrCodeDataUrl);
            // Convert HTML to PDF using Puppeteer (landscape)
            const pdfBuffer = await this.convertHTMLToPDF(html);
            logger_1.default.info("Certificate generated successfully", {
                projectId: options.projectId,
     ...
     */
    static async generateCertificateHTML(options, stats, qrCodeDataUrl) {
  ...
  <div class="certificate-container">
    ...
    static async generateQRCodeDataURL(url) {
        try {
            ...
            logger_1.default.error("Error generating QR code", { error: error.message });
            throw error;
        }
    }
    /**
     ...
     */
    static async generatePreviewImage(html) {
        let browser;
        try {
            browser = await this.launchBrowser();
            const page = await browser.newPage();
            ...
            logger_1.default.error("Error generating preview image", { error: error.message });
            throw error;
        }
    }
    /**
    ...
    static async launchBrowser() {
        ...
        try {
            ...
            logger_1.default.warn("Default Puppeteer launch failed, checking system paths...", { error });
            ...
            for (const candidatePath of systemPaths) {
                if (node_fs_1.default.existsSync(candidatePath)) {
                    logger_1.default.info(`Found system Chrome at ${candidatePath}, attempting launch...`);
                    try {
                        return await puppeteer.launch({
                            headless: true,
                        ...
                        logger_1.default.warn(`Failed to launch system Chrome at ${candidatePath}`, e);
                    }
                }
            }
            ...
            if (home) {
                const puppeteerChromeRoot = node_path_1.default.join(home, ".cache", "puppeteer", "chrome");
                if (node_fs_1.default.existsSync(puppeteerChromeRoot)) {
                    for (const version of node_fs_1.default.readdirSync(puppeteerChromeRoot)) {
                        const candidatePath = node_path_1.default.join(puppeteerChromeRoot, version, "chrome-linux64", "chrome");
                        if (node_fs_1.default.existsSync(candidatePath)) {
                            logger_1.default.info(`Found Puppeteer Chrome at ${candidatePath}, attempting launch...`);
                            try {
                                return await puppeteer.launch({
                                    headless: true,
                                    executablePath: candidatePath,
                                ...
                                logger_1.default.warn(`Failed to launch Puppeteer Chrome at ${candidatePath}`, e);
                            }
                        }
                    }
                }
            ...
            try {
                logger_1.default.info("Retrying with auto-detected path...");
                return await puppeteer.launch({
                    headless: true,
                    args: launchArgs,
                });
                ...
                logger_1.default.error("All browser launch attempts failed.");
                throw finalError;
            }
        }
    }
}
exports.AuthorshipCertificateGenerator = AuthorshipCertificateGenerator;
