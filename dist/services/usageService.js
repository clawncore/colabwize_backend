...
class UsageService {
     ...
     */
    static async trackUsage(userId, feature) {
        const { period_start, period_end } = this.getCurrentPeriod();
        const usage = await prisma_1.prisma.usageTracking.upsert({
            where: {
        ...
        logger_1.default.info("Usage tracked (analytics)", { userId, feature, count: usage.count });
        return usage;
    }
    /**
     ...
     */
    static async getCurrentUsage(userId, subscription) {
        const now = new Date();
        ...
        try {
            if (subscription === undefined) {
                subscription = await subscriptionService_1.SubscriptionService.getUserSubscription(userId);
            }
        ...
        let consumedEvents = [];
        try {
            consumedEvents = await prisma_1.prisma.usageEvent.groupBy({
                by: ["feature"],
                where: {
                    user_id: userId,
                    status: "CONSUMED",
            ...
            logger_1.default.warn("UsageEvent query failed, falling back to usageTracking", {
                error: e.message,
            });
        }
     ...
     * Get usage history
     */
    static async getUsageHistory(userId, months = 3) {
        const now = new Date();
        const startDate = new Date(now.getFullYear(), now.getMonth() - months, 1);
        const usageRecords = await prisma_1.prisma.usageTracking.findMany({
            where: {
                user_id: userId,
    ...
    static async resetMonthlyUsage() {
        ...
        logger_1.default.info("Old usage records cleaned up");
        return true;
    }
}
