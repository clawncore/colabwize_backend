...
class VectorStoreService {
     ...
     */
    static async storeDocuments(documents) {
        ...
        for (const doc of documents) {
            try {
                ...
                await prisma_1.prisma.$executeRawUnsafe(`INSERT INTO documents (content, metadata, embedding) VALUES ($1, $2::jsonb, $3::vector)`, doc.pageContent, JSON.stringify(doc.metadata), `[${embedding.join(",")}]`);
     ...
     */
    static async search(query, filter, k = 5) {
        const apiKey = await secrets_service_1.SecretsService.getOpenAiApiKey();
        if (!apiKey) {
            throw new Error("OPENAI_API_KEY not configured");
        }
        ...
        if (filter) {
            ...
            results = await prisma_1.prisma.$queryRawUnsafe(`SELECT id, content, metadata, 1 - (embedding <=> $1::vector) as similarity
         FROM documents
         WHERE 1 - (embedding <=> $1::vector) > 0.5
         AND metadata @> $2::jsonb
         ORDER BY similarity DESC
         LIMIT $3`, vectorString, JSON.stringify(filter), k);
        }
        else {
         ...
         WHERE 1 - (embedding <=> $1::vector) > 0.5
         ORDER BY similarity DESC
         LIMIT $2`, vectorString, k);
        }
        // Convert results to Document format
        return results.map((row) => new documents_1.Document({
     ...
     */
    static async storeProjectContent(projectId, userId, content) {
        const apiKey = await secrets_service_1.SecretsService.getOpenAiApiKey();
        if (!apiKey) {
            throw new Error("OPENAI_API_KEY not configured");
        }
        ...
        await prisma_1.prisma.$executeRawUnsafe(`DELETE FROM documents WHERE metadata->>'type' = 'project' AND metadata->>'projectId' = $1`, projectId);
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            await prisma_1.prisma.$executeRawUnsafe(`INSERT INTO documents (content, metadata, embedding) VALUES ($1, $2::jsonb, $3::vector)`, chunk, JSON.stringify(metadata), `[${embedding.join(",")}]`);
        }
     ...
     */
    static async searchProjectContext(projectId, query, k = 5) {
        const filter = {
            type: "project",
            projectId,
        };
     ...
     */
    static extractTextFromTiptapJSON(node) {
        if (!node)
            return "";
        let text = "";
        if (node.text) {
