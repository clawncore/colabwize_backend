...
class WaitlistService {
    ...
    static async addToWaitlist(data) {
        try {
            ...
            const existing = await prisma_1.prisma.userFeedback.findFirst({
                where: {
                    type: "waitlist_entry",
            ...
            // Create new entry
            const entry = await prisma_1.prisma.userFeedback.create({
                data: {
                    type: "waitlist_entry",
            ...
            logger_1.default.error("Error adding to waitlist:", error);
     ...
     */
    static async isOnWaitlist(email, feature) {
        try {
            const where = {
                type: "waitlist_entry",
                description: {
            ...
            const entry = await prisma_1.prisma.userFeedback.findFirst({
                where,
            });
            return !!entry;
        }
        catch (error) {
            logger_1.default.error("Error checking waitlist status:", error);
            return false;
        }
    }
    /**
    ...
    static async getPosition(email) {
        try {
            ...
            if (!entry)
                return null;
            const count = await prisma_1.prisma.userFeedback.count({
                where: {
                    type: "waitlist_entry",
                    created_at: {
                        lt: entry.created_at,
    ...
    static async notifyTeam(data) {
        try {
            const webhookUrl = "https://discord.com/api/webhooks/1445349012785074317/tVR9cz3trTXBLU3aThYV1gMcZJ8v0gLX65192h0x0986EaqzZzDpVs4R2AvT9Tt3mtUm"; // Reusing feature team webhook
            const embed = {
            ...
            logger_1.default.error("Failed to notifiy team about waitlist", error);
        }
    }
}
