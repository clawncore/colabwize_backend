...
exports.ExternalVerificationService = void 0;
const academicDatabaseService_1 = require("../academicDatabaseService");
...
class ExternalVerificationService {
    ...
    static async verifyCitationPairs(pairs, userId, options) {
        ...
        console.log(`[ExternalVerification] Cache hits: ${cachedResults.length}, To verify: ${pairsToVerify.length}`);
        ...
        for (let i = 0; i < pairsToVerify.length; i += this.CONCURRENCY_LIMIT) {
            ...
            const batchResults = await Promise.all(batch.map(({ pair, originalIndex }) => this.verifyPairWithTimeout(pair, zoteroCreds).then((result) => {
                ...
                const looksLikeProviderIssue = /timeout|timed out|econnreset|fetch failed|socket hang up|network|unavailable|rate.?limit|429|503/i.test(message);
                logger_1.default.error("Verification error for citation", {
                ...
                return {
                    result: {
                        ...
                        status: (looksLikeProviderIssue ? "RETRY_REQUIRED" : "VERIFICATION_FAILED"),
     ...
     */
    static async verifyPairWithTimeout(pair, zoteroCreds) {
     ...
     */
    static async checkCache(hash) {
        try {
            const { prisma } = require("../../lib/prisma");
            if (!prisma.verificationCache)
                return null;
     ...
     */
    static async storeCache(hash, result) {
        try {
            const { prisma } = require("../../lib/prisma");
            if (!prisma.verificationCache)
                return;
     ...
     */
    static async verifyPair(pair, zoteroCreds) {
        ...
        if (pair.reference.extractedDOI) {
            ...
            if (zoteroCreds?.zotero_api_key && zoteroCreds?.zotero_user_id) {
                ...
                const zoteroItems = await ZoteroService.queryItems(zoteroCreds.zotero_user_id, zoteroCreds.zotero_api_key, pair.reference.extractedDOI);
                if (zoteroItems && zoteroItems.length > 0) {
                    const zItem = zoteroItems[0];
                    console.log(`   🏆 GOLD STANDARD MATCH (Zotero): "${zItem.title}"`);
                    bestMatch = {
                        title: zItem.title,
                        authors: zItem.author?.map((a) => `${a.given} ${a.family}`).join(", "),
            ...
            if (!bestMatch) {
                const doiResult = await academicDatabaseService_1.AcademicDatabaseService.searchByDOI(pair.reference.extractedDOI);
                if (doiResult) {
            ...
            if (!bestMatch) {
                const ssResult = await semanticScholarService_1.SemanticScholarService.getPaperByDoi(pair.reference.extractedDOI);
                if (ssResult) {
        ...
        if (!bestMatch) {
            ...
            if (zoteroCreds?.zotero_api_key && zoteroCreds?.zotero_user_id) {
                const { ZoteroService } = require("../zoteroService");
                const zoteroItems = await ZoteroService.queryItems(zoteroCreds.zotero_user_id, zoteroCreds.zotero_api_key, searchQuery);
                if (zoteroItems && zoteroItems.length > 0) {
                    const zItem = zoteroItems[0];
            ...
            if (!bestMatch) {
                ...
                logger_1.default.info("Verifying citation", { inline: pair.inline.text, query: searchQuery });
                apiResults = await academicSearchService_1.AcademicSearchService.searchPapers(searchQuery, 10);
        ...
        if (!bestMatch) {
            ...
            const looksLikeBookOrArchive = Boolean(pair.reference.extractedDOI) ||
                wordCount <= 12 ||
                /(proceedings|conference|thesis|dissertation|report|archive|museum|library|chapter|edition)/i.test(pair.reference.rawText);
            return {
                inlineLocation,
        ...
        const buildResult = (status, message) => ({
            inlineLocation,
            status,
            existenceStatus: status === "VERIFIED" ? "CONFIRMED" : "NOT_FOUND",
            supportStatus: (semanticSupport?.status || "PENDING"),
            message,
            similarity,
            issues: [],
            foundPaper: {
        ...
        if (similarity < 0.5) {
            return buildResult("VERIFICATION_FAILED", `⚠️ External verification inconclusive (${(similarity * 100).toFixed(0)}% match). Closest paper found: "${bestMatch.title}".`);
        }
        // Tier 2: Fair Match (50% - 70%)
        if (similarity < 0.7) {
            return buildResult("VERIFIED", `✅ Verified (Fair Match: ${(similarity * 100).toFixed(0)}%). Found: "${bestMatch.title}".`);
        }
        // Tier 3: Good Match (> 70%)
        return buildResult("VERIFIED", `✅ Verified: "${bestMatch.title}" (${(similarity * 100).toFixed(0)}% match from ${bestMatch.database})`);
    }
    /**
     * Calculate similarity between reference text and a paper's title + abstract.
     */
    static calculateSimilarity(refText, title, abstract) {
        if (!title)
            return 0;
     ...
     */
    static buildSearchQuery(reference) {
        const parts = [];
...
exports.ExternalVerificationService = ExternalVerificationService;
