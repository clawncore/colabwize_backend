...
class FeedbackService {
    ...
    static async isUserAdmin(userId) {
        try {
            ...
            if (user &&
                (user.user_role === "feedback" || user.email.endsWith("@colabwize.com"))) {
                return true;
            }
    ...
    static async createFeedback(feedbackData) {
        try {
            ...
            // Log the action
            await this.logFeedbackAction(feedback.user_id, "feedback_created", feedback.id, "UserFeedback", {
                type: feedback.type,
                category: feedback.category,
                priority: feedback.priority,
            });
            // Notify administrators
            await this.notifyFeedbackTeam("New User Feedback", `A new ${feedback.type} has been submitted: "${feedback.title}"`);
            return feedback;
        }
        catch (error) {
    ...
    static async getFeedbackItems(userId, filters, limit = 50) {
        try {
            ...
            return await prisma_1.prisma.userFeedback.findMany({
                where: whereClause,
                orderBy: {
                    created_at: "desc",
                },
    ...
    static async getFeedbackById(userId, feedbackId) {
        try {
    ...
    static async updateFeedbackStatus(userId, feedbackId, status, adminNotes) {
        try {
            // Check if user is admin
            const isAdmin = await this.isUserAdmin(userId);
            if (!isAdmin) {
            ...
            // Log the action
            await this.logFeedbackAction(userId, `feedback_${status}`, feedbackId, "UserFeedback", {
                status,
                adminNotes,
            });
            ...
            if (feedback.user_id) {
                ...
                if (user) {
                    ...
                    if (subject && message) {
                        await emailService_1.EmailService.sendNotificationEmail(user.email, user.full_name || "User", subject, message, "feedback");
                    }
                }
            }
            return feedback;
    ...
    static async addFeedbackComment(userId, feedbackId, commentData) {
        try {
            ...
            const comment = await prisma_1.prisma.feedbackComment.create({
                data: {
            ...
            // Log the action
            await this.logFeedbackAction(userId, "feedback_comment_added", feedbackId, "FeedbackComment", {
    ...
    static async getFeedbackComments(userId, feedbackId, includeInternal = false) {
        try {
            ...
            return await prisma_1.prisma.feedbackComment.findMany({
                where: whereClause,
                orderBy: {
                    created_at: "asc",
                },
    ...
    static async logFeedbackAction(userId, action, resourceId, resourceType, details) {
        try {
            ...
            logger_1.default.error("Error logging feedback action:", error);
            // Don't throw error as this is just for logging
        }
    }
    // Notify feedback team via Discord webhook
    static async notifyFeedbackTeam(subject, message) {
        try {
            ...
            const embed = {
                ...
                footer: {
                    text: "ColabWize Feedback System",
                },
            };
            ...
            logger_1.default.info("Feedback team notified via Discord webhook", {
                subject,
                message,
            });
        }
        catch (error) {
            logger_1.default.error("Error notifying feedback team via Discord webhook:", error);
            // Fallback to email notification if Discord fails
            try {
                const feedbackEmail = await secrets_service_1.default.getFeedbackEmail();
                await emailService_1.EmailService.sendNotificationEmail(feedbackEmail, "Feedback Team", subject, message, "feedback");
            }
            catch (emailError) {
    ...
    static async getFeedbackStats(userId) {
        try {
            ...
            const totalFeedback = await prisma_1.prisma.userFeedback.count();
            const openFeedback = await prisma_1.prisma.userFeedback.count({
                where: {
                    status: "open",
                },
            });
            const inProgressFeedback = await prisma_1.prisma.userFeedback.count({
                where: {
                    status: "in_progress",
                },
            });
            const resolvedFeedback = await prisma_1.prisma.userFeedback.count({
                where: {
                    status: "resolved",
                },
            });
            const feedbackByType = await prisma_1.prisma.userFeedback.groupBy({
                by: ["type"],
                _count: {
                    _all: true,
                },
            });
            const feedbackByCategory = await prisma_1.prisma.userFeedback.groupBy({
                by: ["category"],
                _count: {
                    _all: true,
                },
