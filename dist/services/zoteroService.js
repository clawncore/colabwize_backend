...
class ZoteroService {
     ...
     */
    static async fetchLibrary(zoteroUserId, zoteroApiKey, limit = 50, start = 0) {
        try {
            logger_js_1.default.info(`[Zotero Service] Fetching library for user ${zoteroUserId} (limit: ${limit}, start: ${start})`);
     ...
     */
    static async queryItems(zoteroUserId, zoteroApiKey, query) {
        try {
     ...
     */
    static async fetchAttachments(zoteroUserId, zoteroApiKey, itemKey) {
        try {
     ...
     */
    static async fetchFormattedBib(zoteroUserId, zoteroApiKey, itemKey, style = 'apa') {
        try {
     ...
     */
    static async updateItem(zoteroUserId, zoteroApiKey, itemKey, updateData) {
        try {
            const url = `https://api.zotero.org/users/${zoteroUserId}/items/${itemKey}`;
            // Get current version first (important for Zotero updates)
            const currentResponse = await axios_1.default.get(url, {
                headers: { 'Zotero-API-Key': zoteroApiKey }
            });
     ...
     */
    static async createNote(zoteroUserId, zoteroApiKey, parentKey, noteText) {
        try {
            const url = `https://api.zotero.org/users/${zoteroUserId}/items`;
            const noteItem = {
                itemType: 'note',
            ...
            const response = await axios_1.default.post(url, [noteItem], {
                headers: {
                    'Zotero-API-Key': zoteroApiKey,
                    'Content-Type': 'application/json'
                }
     ...
     */
    static async fetchCollections(zoteroUserId, zoteroApiKey) {
        try {
     ...
     */
    static async fetchCollectionItems(zoteroUserId, zoteroApiKey, collectionKey, limit = 50, start = 0) {
        try {
    ...
    static async importItem(colabUserId, projectId, itemData) {
        try {
            ...
            const csl = (0, cslNormalization_js_1.normalizeToCSL)(targetData);
            ...
            if (csl.PMID || csl.pmid)
                identifiers.pmid = csl.PMID || csl.pmid;
            if (csl.PMCID || csl.pmcid)
                identifiers.pmcid = csl.PMCID || csl.pmcid;
...
exports.ZoteroService = ZoteroService;
