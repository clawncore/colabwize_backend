...
exports.CitationMappingService = void 0;
exports.extractAuthors = extractAuthors;
exports.extractYear = extractYear;
exports.extractIEEEFromReference = extractIEEEFromReference;
exports.parseReferenceLine = parseReferenceLine;
...
function extractAuthors(referenceText) {
    if (!referenceText)
        return [];
...
function extractIEEEFromReference(referenceText) {
    const match = referenceText.match(/^\[?(\d+)\]?[\.\s]/);
    return match ? parseInt(match[1], 10) : null;
}
function parseReferenceLine(rawText, id) {
...
function detectFallbackType(text) {
 ...
 */
function stableId(referenceText) {
    return (0, crypto_1.createHash)("sha1").update(referenceText.trim()).digest("hex").substring(0, 36);
}
/**
 * Strip HTML tags while preserving paragraph boundaries.
 * "<p>Hello</p><p>World</p>" → "Hello\nWorld"
 */
function stripHtmlPreserveParagraphs(html) {
    return html
        .replace(/<\/p>/gi, "\n")
        .replace(/<\/div>/gi, "\n")
        .replace(/<\/li>/gi, "\n")
...
class CitationMappingService {
     ...
     */
    static parseDocument(rawContent, format) {
     ...
     */
    static splitBibliography(content, format) {
     ...
     */
    static parseBibliography(bibText) {
        if (!bibText.trim())
            return [];
        const entities = [];
     ...
     */
    static mapInTextCitationsToJSON(bodyText, bibliographyEntities) {
        const usedEntities = new Map();
        // Build IEEE number→entity map
        const ieeeMap = new Map();
     ...
     */
    static appendBibliographyToJSON(bodyJson, usedEntities, allEntities) {
...
exports.CitationMappingService = CitationMappingService;
 ...
 */
function expandIEEEMatches(matches, ieeeMap) {
    const result = [];
    for (const m of matches) {
        ...
        for (const num of numbers) {
            ...
            if (start > 0 && m.text[start - 1] === "[")
                start--;
            if (end < m.text.length && m.text[end] === "]")
                end++;
            const entity = ieeeMap.get(num);
            result.push({
                type: "ieee",
