...
class CredibilityScoreService {
    ...
    static calculateCredibility(paper) {
        ...
        const totalScore = factors.citationScore * weights.citation +
     ...
     */
    static calculateCitationScore(paper) {
        ...
        if (citationsPerYear >= 20)
            return 90;
        if (citationsPerYear >= 10)
            return 80;
        if (citationsPerYear >= 5)
            return 70;
        if (citationsPerYear >= 2)
            return 60;
        if (citationsPerYear >= 1)
            return 50;
        return 30;
    }
    /**
     ...
     */
    static calculateRecencyScore(paper) {
        if (!paper.year)
            return 50;
        const currentYear = new Date().getFullYear();
     ...
     * Peer review score
     */
    static calculatePeerReviewScore(paper) {
        if (paper.isPeerReviewed === true)
            return 100;
        if (paper.isPeerReviewed === false)
            return 20;
     ...
     */
    static calculateJournalScore(paper) {
        if (!paper.journal)
            return 50;
     ...
     */
    static detectFlags(paper, factors) {
        const flags = [];
        // Old paper
     ...
     */
    static getRecommendation(level, flags) {
        if (level === "high") {
     ...
     */
    static batchCalculateCredibility(papers) {
        const results = new Map();
        papers.forEach((paper, index) => {
            const score = this.calculateCredibility(paper);
            results.set(paper.title, score);
        });
        logger_1.default.info("Batch credibility scoring complete", {
...
exports.CredibilityScoreService = CredibilityScoreService;
