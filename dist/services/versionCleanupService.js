...
class VersionCleanupService {
     ...
     */
    async cleanupUserVersions(userId) {
        try {
            logger_1.default.info(`Starting version cleanup for user: ${userId}`);
            ...
            if (retentionDays === -1) {
                logger_1.default.info(`User ${userId} has unlimited version retention, skipping cleanup`);
                return { count: 0, freedSpace: 0 };
            }
            // Calculate cutoff date
            const cutoffDate = new Date();
            cutoffDate.setDate(cutoffDate.getDate() - retentionDays);
            logger_1.default.info(`Cleaning up versions older than ${retentionDays} days (before ${cutoffDate.toISOString()}) for user ${userId}`);
            ...
            for (const project of userProjects) {
                ...
                const versionsToDelete = await prisma_1.prisma.documentVersion.findMany({
                    where: {
                ...
                if (versionsToDelete.length > 0) {
                    logger_1.default.info(`Found ${versionsToDelete.length} versions to delete for project ${project.id}`);
                    ...
                    await prisma_1.prisma.documentVersion.deleteMany({
                        where: {
                            id: {
                    ...
                    logger_1.default.info(`Deleted ${versionsToDelete.length} versions for project ${project.id}, freed ~${freedSpace.toFixed(4)} GB`);
                }
            }
            logger_1.default.info(`Completed version cleanup for user ${userId}: deleted ${totalDeleted} versions, freed ~${totalFreedSpace.toFixed(4)} GB`);
            return {
                count: totalDeleted,
            ...
            logger_1.default.error(`Error during version cleanup for user ${userId}:`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async cleanupAllUsersVersions() {
        try {
            logger_1.default.info("Starting cleanup for all users");
            // Get all users who have document versions
            const usersWithVersions = await prisma_1.prisma.documentVersion.groupBy({
                by: ["user_id"],
                _count: {
                    id: true,
                },
            ...
            logger_1.default.info(`Found ${usersWithVersions.length} users with document versions`);
            let totalDeleted = 0;
            ...
            for (const userGroup of usersWithVersions) {
                try {
                    ...
                    if (totalUsersProcessed % 10 === 0) {
                        logger_1.default.info(`Processed ${totalUsersProcessed}/${usersWithVersions.length} users, deleted ${totalDeleted} versions so far`);
                    }
                }
                catch (error) {
                    logger_1.default.error(`Error cleaning up versions for user ${userGroup.user_id}:`, error);
                    // Continue with other users
                }
            }
            logger_1.default.info(`Completed cleanup for all users: processed ${totalUsersProcessed} users, deleted ${totalDeleted} versions`);
        }
        catch (error) {
            logger_1.default.error("Error during cleanup for all users:", error);
            throw error;
        }
    }
}
exports.default = VersionCleanupService;
