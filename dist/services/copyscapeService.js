...
class CopyscapeService {
    static API_URL = "https://www.copyscape.com/api/";
    /**
    ...
    static async scanText(content) {
        ...
        if (USE_MOCK_DATA) {
            logger_1.default.info("MOCK MODE: Returning test data (no Copyscape API call)");
            // Simulate API delay
            ...
            logger_1.default.info(`MOCK: Generated ${mockMatches.length} matches for ${wordCount} words`);
            return { matches: mockMatches, summary: mockSummary };
        }
        // ============================================
        // END MOCK MODE
        // ============================================
        const username = await secrets_service_1.SecretsService.getCopyscapeUsername();
        const apiKey = await secrets_service_1.SecretsService.getCopyscapeApiKey();
        if (!username || !apiKey) {
            logger_1.default.error("Copyscape credentials missing");
            throw new Error("MISSING_CREDENTIALS");
        }
        try {
            logger_1.default.info("Calling Copyscape API", { length: content.length });
            // Prepare parameters
            ...
            const response = await axios_1.default.post(this.API_URL, params.toString(), {
                headers: {
            ...
            if (typeof data === "string") {
                try {
                    ...
                    logger_1.default.error("Copyscape returned non-JSON response", { data: data.substring(0, 500) });
            ...
            if (data.error) {
                logger_1.default.error("Copyscape API Error", { error: data.error, response: data });
                throw new Error(data.error);
            }
            // Parse response
            const rawMatches = Array.isArray(data.result) ? data.result : (data.result ? [data.result] : []);
            ...
            logger_1.default.error("Copyscape Scan Failed", { error: error.message });
            throw error;
        }
    }
    /**
    ...
    static mapSnippetsToIndices(originalText, matches) {
        ...
        matches.forEach(match => {
            ...
            if (normalizedSnippet.length < 10)
                return; // Ignore very short snippets to avoid false positives
