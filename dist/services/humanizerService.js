...
class HumanizerService {
    static async humanizeText(text) {
        ...
        if (v2 !== text && !this.isTooSimilar(variations, v2))
            variations.push(v2);
        const v3 = text
            .replace(/\bi\b/g, "one")
        ...
        if (v3 !== text && !this.isTooSimilar(variations, v3))
            variations.push(v3);
    ...
    static async rewriteSelection(selection, _surroundingContext) {
    ...
    static isTooSimilar(suggestions, candidate) {
        for (const s of suggestions) {
            if (compareTwoStrings(s, candidate) > 0.85)
                return true;
        }
        return false;
    }
