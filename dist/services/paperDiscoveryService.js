...
exports.PaperDiscoveryService = void 0;
const prisma_1 = require("../lib/prisma");
const logger_1 = __importDefault(require("../monitoring/logger"));
class PaperDiscoveryService {
    static BASE_URL = "https://api.openalex.org/works";
    /**
     ...
     */
    static async searchPapers(query, offset = 0, limit = 10) {
        try {
            logger_1.default.info(`Starting federated search for: ${query}`);
            ...
            const promises = [
                ...
                import("./arxivService.js").then((m) => m.ArxivService.searchPapers(query, limitPerSource)),
                // Source 3: Semantic Scholar (Covers ScienceDirect, Wiley, IEEE, etc.)
                import("./semanticScholarService.js").then((m) => m.SemanticScholarService.searchPapers(query, limitPerSource)),
                // Source 4: PubMed (Medical)
                import("./pubmedService.js").then((m) => m.PubmedService.searchPapers(query, limitPerSource)),
                // Source 5: Google Scholar (via SerpApi)
                import("./searchService.js").then((m) => m.SearchService.scholarSearch("system_fallback", query, limitPerSource, offset)),
                // Source 6: CrossRef (Validation & Metadata)
                import("./crossRefService.js").then((m) => m.CrossRefService.searchPapers(query, limitPerSource)),
            ];
            ...
            results.forEach((result, index) => {
                if (result.status === "fulfilled" && Array.isArray(result.value)) {
                    ...
                    logger_1.default.warn(`Source ${index} failed or returned empty.`);
                }
            });
            // 3. Fallback to Mock if absolutely no results (Network offline?)
            if (allPapers.length === 0) {
                logger_1.default.info(`No papers found for query: ${query}`);
                return [];
            }
    ...
    // Renamed original searchPapers to openAlexSearch for clarity
    static async openAlexSearch(query, offset, limit) {
        try {
            ...
            const fields = "paperId,title,abstract,authors,year,venue,citationCount,url,isOpenAccess,openAccessPdf,publicationTypes,tldr";
    ...
    static async getPaperDetails(paperId) {
        try {
            ...
            const fields = "paperId,title,abstract,authors,year,venue,citationCount,referenceCount,url,isOpenAccess,openAccessPdf,publicationTypes,tldr";
            ...
            if (response.status === 403 && headers["x-api-key"]) {
                logger_1.default.warn("Semantic Scholar API Key rejected (403), retrying without key...");
            ...
            logger_1.default.error(`Error getting paper details for ${paperId}:`, error);
            throw error;
        }
    }
    static calculateConfidenceScore(paper) {
        const currentYear = new Date().getFullYear();
        const age = Math.max(0, currentYear - (paper.year || currentYear));
        // 1. Recency Score (Decay)
        // 0 years = 100, 10 years ~ 20 (0.85^10 = 0.19)
        let recencyScore = Math.max(0, Math.min(100, Math.round(100 * Math.pow(0.85, age))));
        if (!paper.year)
            recencyScore = 0;
        ...
        if (paper.citationCount) {
            ...
            qualityScore = Math.min(100, Math.round(Math.log10(paper.citationCount + 1) * 33));
        }
        ...
        const warnings = [];
        if (age > 10)
            warnings.push("Paper is over 10 years old");
        if ((paper.citationCount || 0) < 5 && age > 2)
            warnings.push("Low citation count for age");
        if (!paper.venue)
            warnings.push("No venue/journal verification");
        return {
            score: totalScore,
            status,
            recencyScore,
     ...
     */
    static async savePaperToDatabase(paper) {
        try {
            // Check if already exists by externalId
            const existing = await prisma_1.prisma.researchPaper.findUnique({
                where: { externalId: paper.externalId },
            });
            if (existing)
                return existing;
            return await prisma_1.prisma.researchPaper.create({
                data: {
            ...
            logger_1.default.error("Error saving paper to DB:", error);
            throw error;
        }
    }
    /**
     ...
     */
    static async getPaperGraph(paperId) {
        try {
            const apiKey = process.env.OPENALEX_API_KEY || "";
            const headers = {};
            if (apiKey)
            ...
            // If 403 Forbidden and we used an API key, retry without the key
            if (response.status === 403 && headers["x-api-key"]) {
                logger_1.default.warn("Semantic Scholar API Key rejected (403), retrying graph fetch without key...");
                response = await fetch(url);
            }
            if (!response.ok)
            ...
            if (data.references) {
                data.references.slice(0, 10).forEach((ref) => {
                    if (!ref.paperId)
                        return;
                    // Add Node
            ...
            if (data.citations) {
                data.citations.slice(0, 10).forEach((cit) => {
                    if (!cit.paperId)
                        return;
                    // Add Node
            ...
            logger_1.default.error("Error generating research graph:", error);
            return {
                nodes: [],
                edges: [],
            };
    ...
    static async savePaperToLibrary(userId, paper, notes) {
        try {
            // 1. Ensure ResearchPaper exists
            let researchPaper = await prisma_1.prisma.researchPaper.findUnique({
                where: { externalId: paper.externalId },
            });
            if (!researchPaper) {
                researchPaper = await prisma_1.prisma.researchPaper.create({
                    data: {
                        externalId: paper.externalId,
                        title: paper.title,
                        abstract: paper.abstract,
            ...
            const savedPaper = await prisma_1.prisma.savedPaper.upsert({
                where: {
                    user_id_paper_id: {
                        user_id: userId,
            ...
            logger_1.default.error("Error saving paper to library:", error);
            throw error;
        }
    }
    // Get user's library
    static async getUserLibrary(userId) {
        try {
            const savedPapers = await prisma_1.prisma.savedPaper.findMany({
                where: {
                    user_id: userId,
                },
                include: {
            ...
            return savedPapers.map((saved) => {
                ...
                const confidenceScore = this.calculateConfidenceScore(paperData);
                return {
                    ...
                    author: Array.isArray(saved.paper.authors)
                        ? saved.paper.authors.map((a) => a.name).join(", ")
                        : "Unknown",
            ...
            logger_1.default.error("Error fetching user library:", error);
            throw error;
        }
    }
    // Remove a paper from the user's library
    static async removePaperFromLibrary(userId, paperId) {
        try {
            ...
            const researchPaper = await prisma_1.prisma.researchPaper.findUnique({
                where: { externalId: paperId },
            });
            if (!researchPaper) {
                throw new Error("Paper not found");
            }
            const deleted = await prisma_1.prisma.savedPaper.delete({
                where: {
                    user_id_paper_id: {
                        user_id: userId,
                        paper_id: researchPaper.id,
            ...
            logger_1.default.error("Error removing paper from library:", error);
            throw error;
        }
    }
}
exports.PaperDiscoveryService = PaperDiscoveryService;
