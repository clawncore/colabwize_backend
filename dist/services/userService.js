...
class UserService {
    ...
    // Get user's AI usage
    static async getUserAIUsage(userId) {
        try {
            return await prisma_1.prisma.aIUsage.findMany({
                where: { user_id: userId },
            });
        }
        catch (error) {
    ...
    // Track AI usage
    static async trackAIUsage(userId, feature, tokensUsed, cost) {
        try {
            ...
            return await prisma_1.prisma.aIUsage.upsert({
                where: {
                    user_id_month_year: {
                        user_id: userId,
                        month,
    ...
    static async getUserProjectsCount(userId) {
        try {
            return await prisma_1.prisma.project.count({
                where: { user_id: userId },
            });
        }
        catch (error) {
            throw new Error(`Error counting user projects: ${error.message}`);
        }
    }
}
