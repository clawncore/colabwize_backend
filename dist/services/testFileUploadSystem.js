...
async function testFileUploadSystem() {
    ...
    try {
        ...
        const mockFileBuffer = Buffer.from("This is a test document for the file upload system.", "utf-8");
        ...
        const uploadResult = await supabaseStorageService_1.SupabaseStorageService.uploadFile(mockFileBuffer, mockFileName, mockMimeType, mockUserId, {
            userId: mockUserId,
        ...
        const downloadedBuffer = await supabaseStorageService_1.SupabaseStorageService.downloadFile(uploadResult.path);
        const downloadedContent = downloadedBuffer.toString("utf-8");
        console.log("✓ File downloaded from Supabase successfully");
        console.log("Downloaded content matches original:", downloadedContent === mockFileBuffer.toString("utf-8"));
        console.log("\n3. Testing database integration...");
        // Check if the file record was created in the database
        const fileRecords = await prisma_1.prisma.file.findMany({
            where: {
                user_id: mockUserId,
        ...
        if (fileRecords.length > 0) {
            ...
            console.log("⚠ No file record found in database (this may be expected if the upload service creates records differently)");
        }
        console.log("\n4. Testing file deletion from Supabase...");
        // Test deleting the file from Supabase
        const deleteResult = await supabaseStorageService_1.SupabaseStorageService.deleteFile(uploadResult.path);
        console.log("✓ File deletion result:", deleteResult);
        console.log("\n5. File upload and storage system summary:");
        console.log("Supabase bucket management is handled internally by the service.");
