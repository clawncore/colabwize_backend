...
async function initializePrisma() {
    ...
    if (!databaseUrl) {
        databaseUrl = await secrets_service_1.SecretsService.getDatabaseUrl();
    }
    if (!databaseUrl) {
        logger_1.default.error("❌ DATABASE_URL not found in environment or secrets");
    ...
    try {
        ...
        logger_1.default.info("Async Database Connection Details:", {
            host: url.hostname,
            port: url.port,
        ...
        logger_1.default.error("❌ Error parsing/configuring DATABASE_URL in async init:", error);
