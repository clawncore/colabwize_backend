 ...
 */
router.post("/extend-plus", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            return res.status(401).json({ error: "Not authenticated" });
...
router.get("/plans", async (req, res) => {
    try {
        const plans = subscriptionService_1.SubscriptionService.getAvailablePlans();
 ...
 */
router.get("/current", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    ...
    try {
        ...
        const withTimeout = (promise, ms) => {
            ...
            return Promise.race([
                promise
                    ...
                    return "TIMEOUT"; // Treat error as timeout/failure for fallback
                }),
                timeoutPromise,
            ]);
        };
        ...
        const executeLogic = async () => {
            ...
            const subResult = await withTimeout(subscriptionService_1.SubscriptionService.getUserSubscription(user.id), DB_TIMEOUT_MS);
            ...
            const [usageResult, creditResult, totalResult] = await Promise.all([
                withTimeout(usageService_1.UsageService.getCurrentUsage(user.id, subscriptionForUsage), DB_TIMEOUT_MS),
                withTimeout(CreditService_1.CreditService.getBalance(user.id), DB_TIMEOUT_MS),
                withTimeout(prisma_1.prisma.project.count({ where: { user_id: user.id } }), DB_TIMEOUT_MS),
            ]);
        ...
        const { subResult, usageResult, creditResult, totalResult } = result;
        // DERIVE STATE
        ...
        plan = await subscriptionService_1.SubscriptionService.getActivePlan(user.id, subscriptionData);
        ...
        const normalizedPlan = subscriptionService_1.SubscriptionService.normalizePlanId(plan);
        ...
        const limits = subscriptionService_1.SubscriptionService.getPlanLimits(normalizedPlan);
        ...
        console.error(isHardTimeout
            ? "CRITICAL: HTTP Hard Timeout in /subscription/current"
            : "Critical error in /subscription/current:", error);
        // FAIL SAFE: Never return 500 for this critical endpoint
        return res.status(200).json({
            success: true,
            status: "unknown",
            plan: "free",
            limits: subscriptionService_1.SubscriptionService.getPlanLimits("free"),
            usage: {},
            creditBalance: 0,
            totalDocuments: 0,
 ...
 */
router.post("/checkout", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    try {
        const user = req.user;
        ...
        const currentSubscription = await subscriptionService_1.SubscriptionService.getUserSubscription(user.id);
        ...
        const config = await secrets_service_1.SecretsService.getLemonSqueezyConfig();
        let variantId;
        if (plan === "plus") {
            variantId =
        ...
        // Create checkout URL
        const checkoutUrl = await lemonSqueezyService_1.LemonSqueezyService.createCheckout({
            variantId,
 ...
 */
router.post("/portal", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            return res.status(401).json({
        ...
        const subscription = await subscriptionService_1.SubscriptionService.getUserSubscription(user.id);
        ...
        const portalUrl = await lemonSqueezyService_1.LemonSqueezyService.getCustomerPortalUrl(subscription.lemonsqueezy_customer_id);
        return res.status(200).json({
            success: true,
            portalUrl,
        });
 ...
 */
router.post("/cancel", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            return res.status(401).json({
        ...
        await subscriptionService_1.SubscriptionService.cancelSubscription(user.id);
        return res.status(200).json({
            success: true,
 ...
 */
router.post("/reactivate", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            return res.status(401).json({
        ...
        await subscriptionService_1.SubscriptionService.reactivateSubscription(user.id);
        return res.status(200).json({
            success: true,
 ...
 */
router.get("/usage", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            return res.status(401).json({
        ...
        const usage = await usageService_1.UsageService.getUsageHistory(user.id, months);
        return res.status(200).json({
            success: true,
            usage,
        });
 ...
 */
router.get("/certificates/retention", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            return res.status(401).json({
        ...
        const { CertificateRetentionService } = await import("../../services/certificateRetentionService.js");
        const retentionInfo = await CertificateRetentionService.getRetentionInfo(user.id);
        return res.status(200).json({
            success: true,
            ...retentionInfo,
        });
 ...
 */
router.get("/payment-methods", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            return res.status(401).json({
        ...
        await subscriptionService_1.SubscriptionService.ensureLemonCustomer(user);
 ...
 */
router.get("/invoices", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            return res.status(401).json({
        ...
        const invoices = await prisma_1.prisma.paymentHistory.findMany({
            where: {
                user_id: user.id,
            },
            orderBy: {
 ...
 */
router.post("/payment-methods/update", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            return res.status(401).json({
        ...
        const customerId = await subscriptionService_1.SubscriptionService.ensureLemonCustomer(user);
        // 2. Generate Portal URL
        // We prioritize the Customer Portal URL which handles all billing needs
        const redirectUrl = await lemonSqueezyService_1.LemonSqueezyService.getCustomerPortalUrl(customerId);
        return res.status(200).json({
            success: true,
            redirectUrl,
        });
 ...
 */
router.post("/credits/auto-use", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    try {
        const user = req.user;
        ...
        await subscriptionService_1.SubscriptionService.updateAutoUseCredits(user.id, enabled);
        return res.status(200).json({
            success: true,
 ...
 */
router.get("/credits/history", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            return res.status(401).json({
        ...
        const history = await prisma_1.prisma.creditTransaction.findMany({
            where: { user_id: user.id },
 ...
 */
router.get("/billing/overview", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            return res.status(401).json({
        ...
        const [subscription, usage, paymentMethods, documentsThisMonth, documentsLastMonth, dailyTrend, creditBalance,] = await Promise.all([
            subscriptionService_1.SubscriptionService.getUserSubscription(user.id),
            usageService_1.UsageService.getCurrentUsage(user.id),
            ...
            (async () => {
                ...
                documents.forEach((doc) => {
                    const daysDiff = Math.floor((Date.now() - new Date(doc.uploaded_at).getTime()) /
                        (1000 * 60 * 60 * 24));
            ...
            CreditService_1.CreditService.getBalance(user.id),
        ]);
        // Get plan info using central service logic
        const plan = await subscriptionService_1.SubscriptionService.getActivePlan(user.id, subscription);
 ...
 */
router.get("/features/:feature", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    try {
        const user = req.user;
        ...
        const hasAccess = await subscriptionService_1.SubscriptionService.checkFeatureAccess(user.id, feature);
        return res.status(200).json({
            success: true,
            hasAccess,
            feature,
