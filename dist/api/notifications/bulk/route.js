...
exports.POST_READ = POST_READ;
exports.POST_DELETE = POST_DELETE;
exports.POST_DISMISS = POST_DISMISS;
exports.POST_SNOOZE = POST_SNOOZE;
const notificationService_1 = require("../../../services/notificationService");
const client_1 = require("../../../lib/supabase/client");
// POST /api/notifications/bulk/read - Mark multiple notifications as read
async function POST_READ(request) {
    try {
        // Get user session
        ...
        for (const notificationId of notificationIds) {
            try {
                const result = await (0, notificationService_1.markNotificationAsRead)(notificationId);
...
async function POST_DELETE(request) {
    try {
        // Get user session
        const supabaseClient = await (0, client_1.getSupabaseClient)();
        if (!supabaseClient) {
        ...
        for (const notificationId of notificationIds) {
            try {
                const result = await (0, notificationService_1.deleteNotification)(notificationId);
                results.push({ notificationId, success: true, result });
            }
            catch (error) {
                results.push({
...
async function POST_DISMISS(request) {
    try {
        // Get user session
        const supabaseClient = await (0, client_1.getSupabaseClient)();
        if (!supabaseClient) {
        ...
        for (const notificationId of notificationIds) {
            try {
                const result = await (0, notificationService_1.dismissNotification)(notificationId);
                results.push({ notificationId, success: true, result });
            }
            catch (error) {
                results.push({
...
async function POST_SNOOZE(request) {
    try {
        // Get user session
        const supabaseClient = await (0, client_1.getSupabaseClient)();
        if (!supabaseClient) {
        ...
        for (const notificationId of notificationIds) {
            try {
                const result = await (0, notificationService_1.snoozeNotification)(notificationId, snoozeDate);
                results.push({ notificationId, success: true, result });
            }
            catch (error) {
                results.push({
