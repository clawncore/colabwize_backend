...
router.get("/list", hybridAuthMiddleware_1.authenticateHybridRequest, listLimiter, async (req, res) => {
    ...
    try {
        ...
        const result = await googleDriveService_1.GoogleDriveService.listFiles(userId, folderId, pageToken);
        res.set("Cache-Control", "no-store, no-cache, must-revalidate");
        res.set("Pragma", "no-cache");
        return res.status(200).json(result);
    }
    catch (error) {
        logger_1.default.error("[Google Drive List Error]:", error.message);
...
router.post("/create-project", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    ...
    try {
        ...
        const { stream, fileName, mimeType } = await googleDriveService_1.GoogleDriveService.getFileContent(userId, fileId);
        ensureUploadDir();
        ...
        const file = {
            ...
            mimetype: mimeType,
            size: fs_1.default.statSync(tempPath).size,
            filename: path_1.default.basename(tempPath),
        };
        const project = await documentUploadService_1.DocumentUploadService.createProjectWithDocument(userId, title, description || "", file, workspaceId, 'google-drive');
        return res.status(201).json({ success: true, data: project });
    }
    catch (error) {
        logger_1.default.error("[Google Drive Create Project Error]:", error.message);
        return handleCloudError(res, error);
    }
    finally {
        if (tempPath) {
...
router.get("/download/:fileId", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    try {
        ...
        logger_1.default.error("[Google Drive Download Error]:", error.message);
        return handleCloudError(res, error);
    }
});
...
router.get("/status", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    const userId = req.user.id;
    try {
        const result = await googleDriveService_1.GoogleDriveService.listFiles(userId, undefined, undefined, 1);
...
router.post("/import", hybridAuthMiddleware_1.authenticateHybridRequest, importLimiter, async (req, res) => {
    ...
    try {
        ...
        const { stream: gdStream, fileName, mimeType } = await googleDriveService_1.GoogleDriveService.getFileContent(userId, fileId);
        if (!fileName) {
        ...
        tempPath = path_1.default.join(UPLOAD_DIR, tempFileName);
        await streamPipeline(gdStream, fs_1.default.createWriteStream(tempPath));
        ...
        logger_1.default.info("Google Drive file imported successfully", { userId, projectId, fileId: fileRecord.id, fileName, fileSize });
        ...
        logger_1.default.error("[Google Drive Import Error]:", error.message);
        return handleCloudError(res, error);
    }
    finally {
        if (tempPath) {
