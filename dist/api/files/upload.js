...
const upload = (0, multer_1.default)({
    ...
    fileFilter: function (req, file, cb) {
        ...
        if (allowedTypes.includes(file.mimetype)) {
            ...
            cb(new Error("Invalid file type. Only PDF, DOC, DOCX, TXT, ZIP, Excel, CSV, and images are allowed."));
        }
    },
});
/**
 ...
 */
router.post("/upload", auth_1.authenticateExpressRequest, upload.single("file"), async (req, res) => {
    try {
        ...
        const uploadResult = await supabaseStorageService_1.SupabaseStorageService.uploadFile(req.file.buffer, fileName || req.file.originalname, req.file.mimetype, userId, {
            userId,
        ...
        logger_1.default.error("File upload error", {
            error: error.message,
            stack: error.stack,
        });
 ...
 */
router.post("/upload-with-project", auth_1.authenticateExpressRequest, upload.single("file"), async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({
        ...
        const project = await prisma_1.prisma.project.create({
            data: {
                user_id: userId,
                title: projectName || path_1.default.parse(req.file.originalname).name,
                description: projectDescription || "",
                citation_style: citationStyle || "apa",
            },
        });
        // Upload file to Supabase storage
        const uploadResult = await supabaseStorageService_1.SupabaseStorageService.uploadFile(req.file.buffer, req.file.originalname, req.file.mimetype, userId, {
            userId,
        ...
        logger_1.default.info("File uploaded and associated with new project", {
            fileId: fileRecord.id,
            projectId: project.id,
            userId,
            fileName: fileRecord.file_name,
        ...
        logger_1.default.error("File upload with project error", {
            error: error.message,
            stack: error.stack,
        });
        // Clean up uploaded file if there was an error
 ...
 */
router.get("/list", auth_1.authenticateExpressRequest, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({
        ...
        const totalCount = await prisma_1.prisma.file.count({
            where: whereClause,
        });
        return res.status(200).json({
            success: true,
        ...
        logger_1.default.error("Get files list error", {
            error: error.message,
            stack: error.stack,
        });
        return res.status(500).json({
...
router.delete("/:id", auth_1.authenticateExpressRequest, async (req, res) => {
    try {
        ...
        logger_1.default.error("Delete file error", {
            error: error.message,
            stack: error.stack,
        });
        return res.status(500).json({
 ...
 * Download a file
 */
router.get("/:id/download", auth_1.authenticateExpressRequest, async (req, res) => {
    try {
        const userId = req.user?.id;
        const fileId = req.params.id;
        if (!userId) {
        ...
        logger_1.default.error("Download file error", {
            error: error.message,
            stack: error.stack,
        });
        return res.status(500).json({
