...
exports.POST_REQUEST_OTP = POST_REQUEST_OTP;
exports.GET = GET;
exports.PUT = PUT;
...
async function POST_REQUEST_OTP(request) {
    try {
        const body = await request.json();
        const action = (0, requestHelpers_1.getSafeString)(body.action);
        const deliveryMethod = (0, requestHelpers_1.getSafeString)(body.delivery_method);
        ...
        const result = await OTPService.sendOTP(user.id, prismaUser.email, "", // No phone number needed
        method, prismaUser.full_name || "", true // isProfileUpdate
        );
        if (!result) {
...
async function PUT(request) {
    try {
        const body = await request.json();
        const full_name = (0, requestHelpers_1.getSafeString)(body.full_name);
        const phone_number = (0, requestHelpers_1.getSafeString)(body.phone_number);
        const user_type = (0, requestHelpers_1.getSafeString)(body.user_type);
        const field_of_study = (0, requestHelpers_1.getSafeString)(body.field_of_study);
        const bio = (0, requestHelpers_1.getSafeString)(body.bio);
        const institution = (0, requestHelpers_1.getSafeString)(body.institution);
        const location = (0, requestHelpers_1.getSafeString)(body.location);
...
async function DELETE(request) {
    try {
        ...
        const adminClient = await (0, client_1.getSupabaseAdminClient)();
        if (adminClient) {
...
async function updateProfileWithOTP(request) {
    try {
        const body = await request.json();
        const email = (0, requestHelpers_1.getSafeString)(body.email);
        const otp = (0, requestHelpers_1.getSafeString)(body.otp);
        const full_name = (0, requestHelpers_1.getSafeString)(body.full_name);
        const phone_number = (0, requestHelpers_1.getSafeString)(body.phone_number);
        const user_type = (0, requestHelpers_1.getSafeString)(body.user_type);
        const field_of_study = (0, requestHelpers_1.getSafeString)(body.field_of_study);
        ...
        if (body.zotero_auto_sync !== undefined)
            updateData.zotero_auto_sync = zotero_auto_sync;
        ...
        if (full_name !== undefined)
            updateDataForSupabase.full_name = full_name;
        if (phone_number !== undefined)
            updateDataForSupabase.phone_number = phone_number;
        if (user_type !== undefined)
            updateDataForSupabase.user_type = user_type;
        if (field_of_study !== undefined)
...
async function changePassword(request) {
    try {
        const body = await request.json();
        const currentPassword = (0, requestHelpers_1.getSafeString)(body.currentPassword);
        const newPassword = (0, requestHelpers_1.getSafeString)(body.newPassword);
        // Get user from authorization header
        const authHeader = request.headers.get("authorization");
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
            return new Response(JSON.stringify({ error: "Missing or invalid authorization header" }), {
        ...
        const signInClient = await (0, client_1.getSupabaseClient)();
...
async function enable2FA(request) {
    try {
...
async function confirm2FA(request) {
    try {
        const user = await getUserFromRequest(request);
        if (!user)
            return createErrorResponse("Unauthorized", 401);
        ...
        // Verify and Enable
        const { backupCodes } = await TwoFactorService_1.TwoFactorService.enable2FA(user.id, secret, token);
        // Send success email
...
// Disable 2FA
async function disable2FA(request) {
    try {
        const user = await getUserFromRequest(request);
        if (!user)
            return createErrorResponse("Unauthorized", 401);
        ...
        const isValid = await TwoFactorService_1.TwoFactorService.validateLogin(user.id, token);
        if (!isValid) {
            return createErrorResponse("Invalid 2FA code", 401);
        }
        // 3. Disable
        await TwoFactorService_1.TwoFactorService.disable2FA(user.id, token);
        return new Response(JSON.stringify({ success: true, message: "2FA disabled successfully" }), { status: 200, headers: { "Content-Type": "application/json" } });
    }
    catch (error) {
...
async function getAccountUsage(request) {
    try {
        // Get user from authorization header
        const authHeader = request.headers.get("authorization");
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
        ...
        const userProjects = await prisma_1.prisma.project.count({
            where: { user_id: user.id },
        });
        const userStorage = await prisma_1.prisma.user.findUnique({
            where: { id: user.id },
            select: { storage_used: true },
        });
        const userCollaborators = await prisma_1.prisma.project.count({
            where: {
                user_id: user.id,
                collaborators: {
                    some: {
        ...
        const aiRequests = await prisma_1.prisma.aIUsage.findFirst({
            where: {
                user_id: user.id,
...
async function hasFeatureAccess(request, feature) {
    try {
        // Get user from authorization header
        const authHeader = request.headers.get("authorization");
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
...
async function getReferralData(request) {
    try {
        // Get user from authorization header
        const authHeader = request.headers.get("authorization");
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
        ...
        const activeRewards = userData.referrals_made.filter((r) => r.reward_status === "granted" && (!r.reward_expires_at || r.reward_expires_at > new Date())).length;
        ...
        return new Response(JSON.stringify({
            ...
            activeRewards,
            totalDaysEarned,
            referrals: userData.referrals_made.map((r) => ({
                id: r.id,
