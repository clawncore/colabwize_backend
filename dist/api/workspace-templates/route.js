...
async function GET(request) {
    try {
        const templates = await prisma_1.prisma.workspaceTemplate.findMany({
        ...
        return new Response(JSON.stringify({ success: false, message: "Failed to fetch workspace templates" }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
}
    ...
    try {
        ...
        if (action === "seed") {
            ...
            for (const template of initialTemplates) {
                await prisma_1.prisma.workspaceTemplate.upsert({
            ...
            for (const template of initialTemplates) {
                const existing = await prisma_1.prisma.workspaceTemplate.findFirst({
                    where: { name: template.name }
                });
                if (existing) {
                    await prisma_1.prisma.workspaceTemplate.update({
                        where: { id: existing.id },
                        data: template
                    });
                }
                else {
                    await prisma_1.prisma.workspaceTemplate.create({
                        data: template
                    });
                }
            }
        ...
        logger_1.default.error("Error seeding workspace templates:", error);
        return new Response(JSON.stringify({ success: false, message: "Failed to seed workspace templates" }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
}
