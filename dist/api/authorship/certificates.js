...
exports.downloadCertificate = exports.getTimeToVerification = exports.updateCertificate = exports.deleteCertificate = exports.createCertificate = exports.getCertificateById = exports.getCertificates = void 0;
...
const getCertificates = async (req, res) => {
    try {
        ...
        const total = await prisma_1.prisma.certificate.count({ where });
        res.json({
            certificates,
            pagination: {
                page: Number(page),
...
const createCertificate = async (req, res) => {
    try {
        ...
        const { project_id, title, file_name, file_path, file_size, certificate_type, metadata, } = req.body;
        ...
        // Update user metrics
        await prisma_1.prisma.userMetrics.update({
        ...
        try {
            if (user.email) {
                ...
                const dashboardUrl = `${await secrets_service_1.SecretsService.getFrontendUrl()}/certificates`;
                await emailService_1.EmailService.sendCertificateReadyEmail(user.email, prismaUser?.full_name || "ColabWize User", title, // Project name/title
                dashboardUrl, retentionDays);
            }
        }
        catch (emailError) {
...
const deleteCertificate = async (req, res) => {
    try {
        ...
        await recycleBinService_1.RecycleBinService.addItemToRecycleBin(user.id, "certificate", {
            id: certificate.id,
...
const getTimeToVerification = async (req, res) => {
    try {
        // Get user from authorization header
        const authHeader = req.headers.authorization;
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
        ...
        const avgVerificationTime = verificationTimes.length > 0
            ? Math.round(verificationTimes.reduce((a, b) => a + b, 0) /
                verificationTimes.length)
            : 0;
        // Get previous average for comparison (from 2 weeks ago)
        const twoWeeksAgo = new Date();
        twoWeeksAgo.setDate(twoWeeksAgo.getDate() - 14);
        const previousCertificates = await prisma_1.prisma.certificate.findMany({
            where: {
                user_id: user.id,
                created_at: {
                    lt: twoWeeksAgo,
        ...
        const previousVerificationTimes = [];
        previousCertificates.forEach((cert) => {
            const createdTime = new Date(cert.created_at).getTime();
            const now = new Date().getTime();
            const timeDiffMinutes = Math.round((now - createdTime) / (1000 * 60));
            previousVerificationTimes.push(timeDiffMinutes);
        });
        const previousAvgVerificationTime = previousVerificationTimes.length > 0
            ? Math.round(previousVerificationTimes.reduce((a, b) => a + b, 0) /
...
exports.getTimeToVerification = getTimeToVerification;
...
const downloadCertificate = async (req, res) => {
    try {
        ...
        const { SupabaseStorageService } = await import("../../services/supabaseStorageService.js");
        // Get signed URL (valid for 60 seconds)
        const signedUrl = await SupabaseStorageService.createSignedUrl(certificate.file_path, 60);
