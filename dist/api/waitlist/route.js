...
router.post("/", async (req, res) => {
    try {
        const { email, feature, name, reason, featureInterest, institution, researchArea, experience, interest, } = req.body;
        ...
        const result = await waitlistService_1.WaitlistService.addToWaitlist({
            email,
            feature: featureName,
            name,
            reason,
        ...
        logger_1.default.error("Error in add to waitlist route:", error);
...
router.get("/check", async (req, res) => {
    try {
        ...
        const isOnWaitlist = await waitlistService_1.WaitlistService.isOnWaitlist(email, feature);
...
router.get("/position", async (req, res) => {
    try {
        ...
        const position = await waitlistService_1.WaitlistService.getPosition(email);
