...
router.get("/email-stats", async (req, res) => {
    try {
        ...
        const [totalSent, totalFailed, totalPending, recent24h, recentErrors] = await Promise.all([
            ...
            prisma_1.prisma.emailLog.count({ where: { sent_at: { gte: last24h } } }),
        ...
        logger_1.default.error("Observability email-stats error:", error);
...
router.get("/auth-stats", async (req, res) => {
    try {
        ...
        const [activeSessions, sessionsLast24h, accountLocks, twoFaEnabled, verifiedUsers, totalUsers,] = await Promise.all([
            prisma_1.prisma.userSession.count({ where: { ended_at: null } }),
            prisma_1.prisma.userSession.count({ where: { created_at: { gte: last24h } } }),
            prisma_1.prisma.accountLock.count({ where: { locked_at: { gte: last24h } } }),
            prisma_1.prisma.user.count({ where: { two_factor_enabled: true } }),
            prisma_1.prisma.user.count({ where: { email_verified: true } }),
        ...
        logger_1.default.error("Observability auth-stats error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
// ==========================================
...
router.get("/user-platform", async (req, res) => {
    try {
        ...
        const [total, verified, activeLast30d, activeLast24h, newLast30d, newPrev30d, withSubscription, withTwoFactor,] = await Promise.all([
            prisma_1.prisma.user.count(),
            prisma_1.prisma.user.count({ where: { email_verified: true } }),
            prisma_1.prisma.user.count({ where: { last_seen_at: { gte: last30d } } }),
            prisma_1.prisma.user.count({ where: { last_seen_at: { gte: last24h } } }),
            prisma_1.prisma.user.count({ where: { created_at: { gte: last30d } } }),
            prisma_1.prisma.user.count({ where: { created_at: { gte: prev30d_start, lte: last30d } } }),
            prisma_1.prisma.user.count({ where: { NOT: { user_type: null } } }), // use user_type as proxy for plan
            prisma_1.prisma.user.count({ where: { two_factor_enabled: true } }),
        ]);
        ...
        logger_1.default.error("Observability user-platform error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
// ==========================================
...
router.get("/research-platform", async (req, res) => {
    try {
        const [totalProjects, activeProjects, completedProjects, archivedProjects, totalPdfs, processingPdfs, completedPdfs, failedPdfs, totalAIUsageRecords,] = await Promise.all([
            ...
            prisma_1.prisma.project.count({ where: { status: "archived" } }),
            prisma_1.prisma.pdfDocument.count(),
            prisma_1.prisma.pdfDocument.count({ where: { status: "processing" } }),
            prisma_1.prisma.pdfDocument.count({ where: { status: "completed" } }),
            prisma_1.prisma.pdfDocument.count({ where: { status: "failed" } }),
        ...
        logger_1.default.error("Observability research-platform error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
// ==========================================
...
router.get("/ai-platform", async (req, res) => {
    try {
        ...
        logger_1.default.error("Observability ai-platform error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
// ==========================================
...
router.get("/background-services", async (req, res) => {
    try {
        ...
        logger_1.default.error("Observability background-services error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
// ==========================================
...
router.get("/database-detail", async (req, res) => {
    try {
        ...
        const [sessionCount, accountLockCount, userCount] = await Promise.all([
            prisma_1.prisma.userSession.count({ where: { ended_at: null } }),
            prisma_1.prisma.accountLock.count(),
            prisma_1.prisma.user.count(),
        ]);
        const migrationList = await prisma_1.prisma.$queryRaw `
    ...
    `;
        ...
        logger_1.default.error("Observability database-detail error:", error);
...
router.get("/integration-health", async (req, res) => {
    try {
        ...
        logger_1.default.error("Observability integration-health error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
// ==========================================
...
router.get("/platform-health", async (req, res) => {
    try {
        ...
        logger_1.default.error("Observability platform-health error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
// ==========================================
...
router.get("/alerts", async (req, res) => {
    try {
        ...
        logger_1.default.error("Observability alerts error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
// ==========================================
...
router.get("/payment-monitoring", async (req, res) => {
    try {
        ...
        logger_1.default.error("Observability payment-monitoring error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
