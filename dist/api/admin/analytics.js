...
router.get("/platform", async (req, res) => {
    try {
        ...
        const [totalUsers, activeUsers, totalDocuments, activeSessions] = await Promise.all([
            ...
            prisma_1.prisma.user.count({ where: { last_seen_at: { gte: thirtyDaysAgo } } }),
            prisma_1.prisma.project.count(),
            prisma_1.prisma.userSession.count({ where: { ended_at: null } }),
        ]);
        ...
        let errorTotal = 0;
        try {
            const apiMetrics = await prisma_1.prisma.platformMetric.findMany({
            ...
            const [apiCallsAgg, errorAgg] = await Promise.all([
                prisma_1.prisma.platformMetric.aggregate({
            ...
            apiTotal = apiMetrics.reduce((sum, m) => sum + m.value, 0);
            errorTotal = errorAgg._sum.value ?? 0;
        }
        catch (err) {
            logger_1.default.warn(`platform_metrics table unavailable, using fallback: ${err.message}`);
        }
        const overview = {
            totalUsers,
            activeUsers,
        ...
        logger_1.default.error("Analytics platform error:", error);
...
router.get("/user-growth", async (req, res) => {
    try {
        ...
        const before = await prisma_1.prisma.user.count({ where: { created_at: { lt: since } } });
        ...
        logger_1.default.error("Analytics user-growth error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
// ────────────────────────────────────────────────
...
router.get("/daily", async (req, res) => {
    try {
        ...
        let apiRows = [];
        try {
            apiRows = await prisma_1.prisma.platformMetric.findMany({
                where: { category: "api", recordedAt: { gte: since } },
                select: { recordedAt: true, value: true },
            });
        }
        catch (err) {
            logger_1.default.warn(`platform_metrics table unavailable for daily series: ${err.message}`);
        }
        for (const m of apiRows) {
            const key = m.recordedAt.toISOString().slice(0, 10);
            const entry = bucket.get(key);
            if (entry)
                entry.apiCalls += Math.round(m.value);
        }
        const data = Array.from(bucket.entries()).map(([date, v]) => ({ date, ...v }));
        res.json({ success: true, data: { data } });
    }
    catch (error) {
        logger_1.default.error("Analytics daily error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
// ────────────────────────────────────────────────
...
router.get("/funnel", async (req, res) => {
    try {
        ...
        const [totalUsers, activeUsers, totalDocs, newUsers, paidSubs, currentPaidSubs] = await Promise.all([
            prisma_1.prisma.user.count(),
            prisma_1.prisma.user.count({ where: { last_seen_at: { gte: since } } }),
            prisma_1.prisma.project.count({ where: { created_at: { gte: since } } }),
            prisma_1.prisma.user.count({ where: { created_at: { gte: since } } }),
            prisma_1.prisma.subscription.count({ where: { created_at: { gte: since } } }),
            prisma_1.prisma.subscription.count({ where: { status: { in: ["active", "on_trial", "trialing"] } } }),
        ]);
        ...
        try {
            ...
            logger_1.default.warn(`GA4 unavailable for funnel, using platform-only: ${err.message}`);
        }
        const toFunnel = (steps) => {
        ...
        logger_1.default.error("Analytics funnel error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
// ────────────────────────────────────────────────
...
router.get("/journey", async (req, res) => {
    try {
        ...
        try {
            ...
            logger_1.default.warn(`GA4 unavailable for journey, using platform-only: ${err.message}`);
        }
        const rawSteps = [
        ...
        logger_1.default.error("Analytics journey error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
// ────────────────────────────────────────────────
...
router.get("/conversions", async (req, res) => {
    try {
        ...
        const [subscriptions, payments, totalUsers] = await Promise.all([
            prisma_1.prisma.subscription.findMany({ select: { plan: true, status: true, created_at: true } }),
            prisma_1.prisma.paymentHistory.findMany({ select: { amount: true, description: true, created_at: true } }),
            prisma_1.prisma.user.count(),
        ]);
        ...
        for (const s of subscriptions) {
            ...
            if (s.status === "active" || s.status === "on_trial" || s.status === "trialing")
                paidCount += 1;
        }
        ...
        let gaConversions = [];
        try {
            const ga = await googleAnalyticsService_1.gaService.getEvents();
            for (const row of ga.rows || []) {
                const values = row.metricValues || [];
                gaConversions.push({
            ...
            logger_1.default.warn(`GA4 events unavailable for conversions: ${err.message}`);
        }
        const data = [
            {
                id: "sub_plans",
        ...
        logger_1.default.error("Analytics conversions error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
// ────────────────────────────────────────────────
...
router.get("/api", async (req, res) => {
    try {
        ...
        let rows = [];
        try {
            rows = await prisma_1.prisma.platformMetric.groupBy({
                by: ["metricName"],
                where: { category: "api", recordedAt: { gte: since } },
                _sum: { value: true },
                _count: true,
            ...
            logger_1.default.warn(`platform_metrics table unavailable, returning empty API metrics: ${err.message}`);
        }
        ...
        logger_1.default.error("Analytics api error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
// ────────────────────────────────────────────────
...
router.get("/errors", async (req, res) => {
    try {
        ...
        try {
            ...
            logger_1.default.warn(`platform_metrics table unavailable, returning empty error metrics: ${err.message}`);
        }
        const totalErrors = rows.reduce((sum, r) => sum + (r._sum?.value ?? 0), 0);
        const data = rows.map((r) => ({
        ...
        logger_1.default.error("Analytics errors error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
// ────────────────────────────────────────────────
...
router.get("/usage", async (req, res) => {
    try {
        ...
        const since = new Date();
        since.setDate(since.getDate() - days);
        let rows = await prisma_1.prisma.usageTracking.groupBy({
            by: ["feature"],
        ...
        if (rows.length === 0) {
            rows = await prisma_1.prisma.usageTracking.groupBy({
                by: ["feature"],
                _sum: { count: true },
                _count: true,
                orderBy: { feature: "asc" },
        ...
        logger_1.default.error("Analytics usage error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
// ────────────────────────────────────────────────
...
router.get("/resources", async (req, res) => {
    try {
        ...
        const data = [
            ...
            {
                name: "Load Average",
                value: Number(os_1.default.loadavg()[0].toFixed(2)),
                unit: "load",
                maxValue: os_1.default.cpus().length,
                status: os_1.default.loadavg()[0] / os_1.default.cpus().length > 1 ? "warning" : "healthy",
            },
        ];
        res.json({ success: true, data: { data } });
    }
    catch (error) {
        logger_1.default.error("Analytics resources error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
// ────────────────────────────────────────────────
...
router.get("/operations", async (req, res) => {
    try {
        ...
        const dir = (change) => (change > 0 ? "up" : change < 0 ? "down" : "flat");
        const [totalUsers, newUsers, newUsersPrev, activeUsers, activeUsersPrev, docs30, docsPrev, payments30, paymentsPrev] = await Promise.all([
            prisma_1.prisma.user.count(),
            prisma_1.prisma.user.count({ where: { created_at: { gte: d30 } } }),
            prisma_1.prisma.user.count({ where: { created_at: { gte: d60, lt: d30 } } }),
            prisma_1.prisma.user.count({ where: { last_seen_at: { gte: d30 } } }),
            prisma_1.prisma.user.count({ where: { last_seen_at: { gte: d60, lt: d30 } } }),
            prisma_1.prisma.project.count({ where: { created_at: { gte: d30 } } }),
            prisma_1.prisma.project.count({ where: { created_at: { gte: d60, lt: d30 } } }),
            prisma_1.prisma.paymentHistory.count({ where: { created_at: { gte: d30 } } }),
            prisma_1.prisma.paymentHistory.count({ where: { created_at: { gte: d60, lt: d30 } } }),
        ]);
        ...
        try {
            [emails30, emailsPrev] = await Promise.all([
                prisma_1.prisma.emailLog.count({ where: { sent_at: { gte: d30 } } }),
                prisma_1.prisma.emailLog.count({ where: { sent_at: { gte: d60, lt: d30 } } }),
            ]);
        }
        catch (err) {
            logger_1.default.warn(`email_logs table unavailable, using fallback: ${err.message}`);
        }
        try {
            supportOpen = await prisma_1.prisma.supportMessage.count({ where: { status: "open" } });
        }
        catch (err) {
            logger_1.default.warn(`support_messages table unavailable, using fallback: ${err.message}`);
        }
        let backupsFailed = 0;
        try {
            backupsFailed = await prisma_1.prisma.backupRecord.count({ where: { status: "failed" } });
        }
        catch (err) {
            logger_1.default.warn(`backup_records table unavailable, using fallback: ${err.message}`);
        }
        ...
        logger_1.default.error("Analytics operations error:", error);
        res.status(500).json({ success: false, error: error.message });
    }
});
