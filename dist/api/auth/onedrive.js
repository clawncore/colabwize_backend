 ...
 */
router.get("/", hybridAuthMiddleware_1.authenticateHybridRequest, oauthInitLimiter, (req, res) => {
...
router.get("/callback", oauthCallbackLimiter, async (req, res) => {
    ...
    try {
        ...
        await prisma_1.prisma.user.update({
            where: { id: userId },
            data: {
                onedrive_access_token: tokenCrypto_1.TokenCrypto.encrypt(tokens.access_token),
                onedrive_refresh_token: tokens.refresh_token
                    ? tokenCrypto_1.TokenCrypto.encrypt(tokens.refresh_token)
                    : null,
...
router.post("/disconnect", hybridAuthMiddleware_1.authenticateHybridRequest, async (req, res) => {
    try {
        ...
        if (user?.onedrive_access_token) {
            try {
                const accessToken = tokenCrypto_1.TokenCrypto.decryptOrPlaintext(user.onedrive_access_token);
                // Revoke the token with Microsoft
                const revokeRes = await fetch(`https://login.microsoftonline.com/common/oauth2/v2.0/logout?post_logout_redirect_uri=${encodeURIComponent(process.env.FRONTEND_URL || "http://localhost:3000")}`, { method: "GET" });
