...
const route_1 = __importDefault(require("./tasks/route"));
const route_2 = __importDefault(require("./tasks/subtasks/route"));
const labels_route_1 = __importDefault(require("./labels-route"));
const views_route_1 = __importDefault(require("./views-route"));
const custom_fields_route_1 = __importDefault(require("./custom-fields-route"));
...
// Sub-routers
router.use("/", activity_1.default); // Handles /:id/activity
router.use("/:workspaceId/custom-fields", custom_fields_route_1.default);
router.use("/", labels_route_1.default);
router.use("/tasks/subtasks", route_2.default);
router.use("/tasks", route_1.default);
router.use("/:workspaceId/views", views_route_1.default);
// GET /api/workspaces/:id/analytics - Get workspace analytics with project metrics
router.get("/:id/analytics", checkWorkspaceRole("editor"), async (req, res) => {
    try {
        const { id } = req.params;
        const { WorkspaceAnalyticsService, } = require("../../services/WorkspaceAnalyticsService");
        const analytics = await WorkspaceAnalyticsService.getWorkspaceAnalyticsWithProjects(id);
        res.json(analytics);
    }
    catch (error) {
        logger_1.default.error("Error fetching workspace analytics", error);
...
router.get("/:id/members", async (req, res) => {
    try {
        ...
        logger_1.default.error("Error fetching workspace members", error);
...
router.get("/:id/files", async (req, res) => {
    try {
        const { id } = req.params;
        const files = await prisma_1.prisma.taskAttachment.findMany({
            where: {
                task: {
                    workspace_id: id,
                },
        ...
        logger_1.default.error("Error fetching workspace files", error);
...
router.get("/:id/overview", async (req, res) => {
    try {
        ...
        const [activeProjectsCount, completedProjectsCount, tasksCounts, myTasks, recentActivity, membersCount, recentProjects,] = await Promise.all([
            // 1. Projects Counts
            ...
            prisma_1.prisma.workspaceTask.groupBy({
                by: ["status"],
                where: {
                    workspace_id: id,
                    is_template: false,
            ...
            // 3. My Tasks (Due soon)
            prisma_1.prisma.workspaceTask.findMany({
                where: {
                    workspace_id: id,
            ...
            // 5. Members Count
            prisma_1.prisma.workspaceMember.count({
                where: { workspace_id: id },
            }),
            // 6. Recent Projects
        ...
        logger_1.default.error("Error fetching workspace overview", error);
...
router.get("/:id/metrics", async (req, res) => {
    try {
        const { id } = req.params;
        const { WorkspaceTaskService, } = require("../../services/workspaceTaskService");
        ...
        logger_1.default.error("Error fetching workspace metrics", error);
        res.status(500).json({ error: "Failed to fetch metrics" });
    }
});
// POST /api/workspaces/:id/notify - Broadcast a notification to all workspace members
router.post("/:id/notify", checkWorkspaceRole("editor"), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        await (0, notificationService_1.broadcastGenericNotification)(id, userId, type, title, message, data);
        res.json({ success: true, message: "Workspace notification broadcasted" });
    }
    catch (error) {
        logger_1.default.error("Error broadcasting workspace notification", error);
...
router.post("/", async (req, res) => {
    try {
        ...
        const activeWorkspacesCount = await prisma_1.prisma.workspace.count({
            where: { owner_id: userId },
        });
        if (plan === "free" && activeWorkspacesCount >= 1) {
            return res.status(403).json({
                error: "WORKSPACE_LIMIT_REACHED",
                message: "Free plan allows a maximum of 1 workspace. Please upgrade your plan to create more workspaces.",
            });
        }
        else if (plan === "plus" && activeWorkspacesCount >= 5) {
            return res.status(403).json({
                error: "WORKSPACE_LIMIT_REACHED",
                message: "Plus plan allows a maximum of 5 workspaces. Please upgrade your plan to create more workspaces.",
            });
        }
        ...
        if (templateId) {
            const template = await prisma_1.prisma.workspaceTemplate.findUnique({
                where: { id: templateId },
            });
            if (template) {
                // Apply Labels
        ...
        await workspaceActivityService_1.WorkspaceActivityService.logActivity(workspace.id, userId, "WORKSPACE_CREATED", {
            name: workspace.name,
...
router.get("/:id", async (req, res) => {
    try {
        ...
        logger_1.default.error("Error fetching workspace", error);
        res.status(500).json({ error: "Failed to fetch workspace" });
    }
});
// PUT /api/workspaces/:id - Update workspace settings (owner or admin only)
router.put("/:id", checkWorkspaceRole("admin"), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
        ...
        await workspaceActivityService_1.WorkspaceActivityService.logActivity(id, userId, "SETTINGS_UPDATED", { changes: req.body }, req.ip);
        res.json(updated);
    }
    catch (error) {
        logger_1.default.error("Error updating workspace", error);
        res.status(500).json({ error: "Failed to update workspace" });
    }
});
// DELETE /api/workspaces/:id - Delete workspace (owner only)
router.delete("/:id", checkWorkspaceRole("admin"), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
        ...
        logger_1.default.error("Error deleting workspace", error);
        res.status(500).json({ error: "Failed to delete workspace" });
    }
});
// POST /api/workspaces/:id/invite - Invite a member by email
router.post("/:id/invite", checkWorkspaceRole("admin"), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
        ...
        const ownerSubscription = await prisma_1.prisma.subscription.findUnique({
            where: { user_id: ownerId },
        });
        const plan = ownerSubscription?.plan?.toLowerCase() || "free";
        // Count existing members (excluding owner)
        const existingMembersCount = await prisma_1.prisma.workspaceMember.count({
            where: {
                workspace_id: id,
                user_id: { not: ownerId },
            },
        ...
        const pendingInvitationsCount = await prisma_1.prisma.workspaceInvitation.count({
            where: {
                workspace_id: id,
                status: "pending",
            },
        ...
        if (plan === "free" && totalCollaborators >= 2) {
            return res.status(403).json({
                ...
                message: "Free plan allows a maximum of 2 collaborators per workspace. The workspace owner must upgrade to invite more members.",
            });
        }
        else if (plan === "plus" && totalCollaborators >= 10) {
            return res.status(403).json({
                error: "COLLABORATOR_LIMIT_REACHED",
                message: "Plus plan allows a maximum of 10 collaborators per workspace. The workspace owner must upgrade to invite more members.",
            });
        }
        // Premium plan is unlimited
        // --- End Subscription Limits Check ---
        // Check if user exists (for notification purposes only)
        const userToInvite = await prisma_1.prisma.user.findUnique({ where: { email } });
        ...
        const existingInvite = await prisma_1.prisma.workspaceInvitation.findUnique({
        ...
        const invitation = await prisma_1.prisma.workspaceInvitation.create({
            data: {
                workspace_id: id,
                email,
                role,
        ...
        await workspaceActivityService_1.WorkspaceActivityService.logActivity(id, userId, "INVITATION_SENT", { email, role }, req.ip);
        // Notify invited user if they already exist in the system
        if (userToInvite) {
            try {
                const { createNotification, } = require("../../services/notificationService");
                await createNotification(userToInvite.id, "collaboration_invite", "New Workspace Invitation", `You have been invited to join the workspace "${workspace.name}" as a ${role}.`, { workspaceId: id, invitationId: invitation.id });
            }
            catch (notifError) {
                logger_1.default.error("Failed to send invitation notification:", notifError);
            }
        }
        // Send email notification
        try {
            const EmailService = require("../../services/emailService").EmailService;
            const SecretsService = require("../../services/secrets-service").default;
        ...
        logger_1.default.error("Error inviting member", error);
        res.status(500).json({ error: "Failed to send invitation" });
    }
});
// PUT /api/workspaces/:id/members/:memberId - Update member role
router.put("/:id/members/:memberId", checkWorkspaceRole("admin"), async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        await workspaceActivityService_1.WorkspaceActivityService.logActivity(id, userId, "MEMBER_ROLE_UPDATED", { memberId, role, memberEmail: updated.user.email }, req.ip);
        // Broadcast role change notification to all workspace members
        try {
            const { broadcastWorkspaceNotification, } = require("../../services/notificationService");
            await broadcastWorkspaceNotification(id, userId, "permission_change", {
                actor: () => ({
                    title: "Role Updated",
                ...
                others: (actorName, recipientName) => ({
                    title: "Member Role Updated",
                    message: `${actorName} changed ${recipientName}'s role to ${role} in "${workspace.name}".`,
                }),
            }, { workspaceId: id }, [updated.user_id]);
        }
        catch (notifError) {
            logger_1.default.error("Failed to send role update notification:", notifError);
        }
        res.json(updated);
    }
    catch (error) {
...
router.delete("/:id/members/:memberId", checkWorkspaceRole("admin"), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id, memberId } = req.params;
        // Previous manual checks removed as middleware handles base permission
        ...
        await workspaceActivityService_1.WorkspaceActivityService.logActivity(id, userId, "MEMBER_REMOVED", { memberId, memberUserId: targetMember.user_id }, req.ip);
        ...
        try {
            ...
            logger_1.default.error("Failed to send workspace removal email", emailError);
        }
...
router.get("/:id/invitations", checkWorkspaceRole("admin"), async (req, res) => {
    try {
        const { id } = req.params;
        const invitations = await prisma_1.prisma.workspaceInvitation.findMany({
            where: { workspace_id: id },
            include: {
                inviter: {
                    select: { id: true, full_name: true, email: true },
...
router.delete("/:id/invitations/:invitationId", checkWorkspaceRole("admin"), async (req, res) => {
    try {
        const { id, invitationId } = req.params;
        await prisma_1.prisma.workspaceInvitation.delete({
            where: {
                id: invitationId,
                workspace_id: id,
            },
...
router.get("/invitations/pending", async (req, res) => {
    try {
        ...
        logger_1.default.error("Error fetching pending invitations", error);
...
router.post("/invitations/:token/accept", async (req, res) => {
    try {
        ...
        const invitation = await prisma_1.prisma.workspaceInvitation.findUnique({
            where: { token },
        ...
        if (invitation.expires_at < new Date()) {
            await prisma_1.prisma.workspaceInvitation.update({
        ...
        await workspaceActivityService_1.WorkspaceActivityService.logActivity(invitation.workspace_id, userId, "MEMBER_JOINED", { email: userEmail, role: invitation.role }, req.ip);
        // Notify workspace owner that someone joined
        try {
            const { createNotification, } = require("../../services/notificationService");
            await createNotification(invitation.workspace.owner_id, "new_collaborator", "New Member Joined", `${req.user.full_name || userEmail} has joined the workspace "${invitation.workspace.name}"`, { workspaceId: invitation.workspace_id });
        }
        catch (notifError) {
            logger_1.default.error("Failed to send join notification:", notifError);
        }
        res.json({
...
router.post("/invitations/:token/decline", async (req, res) => {
    try {
        ...
        logger_1.default.error("Error declining invitation", error);
