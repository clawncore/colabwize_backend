...
async function fetchBytes(src, timeoutMs) {
...
function detectColorSpace(bytes, ext) {
    const e = ext ?? "";
    if (e === "jpg" || e === "jpeg") {
    ...
    if (e === "tif" || e === "tiff") {
        ...
        const read16 = (off) => (little ? bytes.readUInt16LE(off) : bytes.readUInt16BE(off));
        const read32 = (off) => (little ? bytes.readUInt32LE(off) : bytes.readUInt32BE(off));
...
function tryImageSize(bytes, ext) {
    try {
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        const sizeOf = require("image-size");
        const d = sizeOf(bytes, ext ? { type: ext } : undefined);
 ...
 */
async function extractAssets(doc, opts = {}) {
