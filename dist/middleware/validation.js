...
exports.validateSSOConfiguration = validateSSOConfiguration;
exports.validateIPRestrictions = validateIPRestrictions;
// Validation middleware for SSO configuration
function validateSSOConfiguration(req, res, next) {
    const { enabled, provider, metadataUrl, entityId, clientId, clientSecret } = req.body;
    ...
    if (enabled === true) {
        ...
        if (provider === "saml") {
            ...
            if (!metadataUrl && !entityId) {
                res.status(400).json({
                    success: false,
                    message: "Either metadata URL or entity ID is required for SAML provider",
                });
                return;
            }
        }
...
function validateIPRestrictions(req, res, next) {
