...
exports.normalizeToCSL = normalizeToCSL;
function normalizeToCSL(input) {
    if (!input)
        return { id: "unknown", type: "article-journal", title: "Unknown", author: [{ literal: "Unknown" }], issued: { "date-parts": [[new Date().getFullYear()]] } };
    let data = input;
                ...
                if (typeof a === 'string') {
                    ...
                    if (a.includes(';') || (a.includes(',') && a.split(',').length > 1)) {
                        ...
                        a.split(sep).forEach(p => {
                            if (p.trim())
                                item[field].push({ literal: p.trim() });
                        });
                    }
                    else {
