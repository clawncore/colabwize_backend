...
exports.normalizeZoteroItem = normalizeZoteroItem;
exports.normalizeMendeleyItem = normalizeMendeleyItem;
exports.normalizeManualInput = normalizeManualInput;
exports.normalizeCitation = normalizeCitation;
const cslNormalization_js_1 = require("./cslNormalization.js");
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
function normalizeZoteroItem(item, _userId) {
    ...
    citation.attachments = rawAttachments.map((att) => ({
...
// Mendeley normalizer
// ---------------------------------------------------------------------------
function normalizeMendeleyItem(item, _userId) {
    const raw = item ?? {};
    const providerId = raw?.id ?? null;
    ...
    if (Array.isArray(raw?.authors)) {
        cslInput.author = raw.author.map((a) => ({
    ...
    if (raw?.identifiers && typeof raw.identifiers === 'object') {
        ...
        if (raw.identifiers.isbn)
            cslInput.isbn = raw.identifiers.isbn;
        if (raw.identifiers.issn)
            cslInput.issn = raw.identifiers.issn;
        if (raw.identifiers.pmid)
            cslInput.pmid = raw.identifiers.pmid;
        if (raw.identifiers.pmcid)
            cslInput.pmcid = raw.identifiers.pmcid;
        if (raw.identifiers.arxiv)
            cslInput.arxiv = raw.identifiers.arxiv;
    }
    ...
    const csl = (0, cslNormalization_js_1.normalizeToCSL)(cslInput);
    ...
    const mappedAuthors = (cslInput.author ?? csl.author ?? []).map((a) => ({
        family: a?.family,
        given: a?.given,
        literal: a?.literal,
    }));
    ...
    citation.attachments = rawFiles.map((f) => ({
...
// ---------------------------------------------------------------------------
function normalizeManualInput(data) {
    const raw = data ?? {};
    const providerId = raw?.id ?? null;
    ...
    const cslAuthors = (csl.author ?? []).map((a) => ({
        family: a?.family,
        given: a?.given,
        literal: a?.literal,
    }));
    ...
    // Content
    citation.abstract = raw?.abstract ?? csl.abstract ?? null;
    citation.keywords = Array.isArray(raw?.keywords) ? raw.keywords.filter((k) => typeof k === 'string') : [];
    citation.tags = Array.isArray(raw?.tags) ? raw.tags.filter((t) => typeof t === 'string') : [];
    return citation;
}
// ---------------------------------------------------------------------------
// Unified dispatcher
// ---------------------------------------------------------------------------
function normalizeCitation(source, provider, userId) {
    switch (provider) {
        case 'zotero':
