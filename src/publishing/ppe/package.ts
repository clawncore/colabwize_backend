 ...
 *
 * Orchestrates the Publication Export Engine: assign stable ids, index
 * cross-references, extract + quality-check images, then assemble the
 * publisher-compliant package (Manuscript / Figures / Tables / References /
 * Metadata / manifest / cover letter / audit report + Images/) as a list of
 * `PackageFile`s. The adapter zips these into `Submission.zip`.
 */
import type { BlockNode, CanonicalDocument, OutputFormat, Reference } from "../cdm";
import { cdmToHtml } from "../serializers/html";
import type { GenResult } from "../types";
import { assignStableIds } from "./ids";
import { buildCrossReferenceIndex } from "./xref";
import { extractAssets, type ExtractAssetsOptions, type ExtractedAsset } from "./assets";
import { checkImageQuality } from "./quality";
import { buildPlaceholderLabels } from "./serializers/placeholder";
import { getPublisherProfile, type PublisherProfile } from "./profiles";
import type {
  AuditReport,
  BuiltPackage,
  ExportFinding,
...
export interface PackageBuildDeps {
  ...
  extractAssetsOptions?: ExtractAssetsOptions;
}
...
function subDoc(doc: CanonicalDocument, body: BlockNode[]): CanonicalDocument {
  return {
    schemaVersion: "1.0",
...
async function renderWithFallback(
  ...
  const ext =
    res.format === "latex" ? "tex" : res.format === "pdf" ? "pdf" : res.format === "html" ? "html" : "docx";
  if (ext !== format && format !== "html") {
    opts.warnings?.push(
      `Pandoc unavailable — rendered "${format}" content as .${ext} instead. ` +
...
function cslYear(csl: Record<string, unknown>): string | undefined {
...
function bibType(cslType: unknown): string {
...
function toBibtex(refs: Reference[]): string {
  return refs
    .map((r) => {
      const csl = (r.cslJson ?? {}) as Record<string, unknown>;
      ...
      const push = (name: string, value: string | undefined): void => {
        if (value) fields.push(`  ${name} = {${value.replace(/[{}]/g, "")}}`);
      };
      ...
      const authors = Array.isArray(csl.author)
        ? (csl.author as Array<Record<string, unknown>>)
            ...
            .filter(Boolean)
        : [];
      if (authors.length) push("author", authors.join(" and "));
...
function auditToMarkdown(report: AuditReport, systemWarnings: string[] = []): string {
  ...
  if (systemWarnings.length > 0) {
    ...
    for (const w of systemWarnings) lines.push(`- ${w}`);
  }
  return lines.join("\n");
}
...
export async function buildSubmissionPackage(
  doc: CanonicalDocument,
  profileOrId: PublisherProfile | string,
  settings: PpeSettings,
  deps: PackageBuildDeps,
): Promise<BuiltPackage> {
  const profile = typeof profileOrId === "string" ? getPublisherProfile(profileOrId) : profileOrId;
  ...
  const assets = await extractAssets(augmented, deps.extractAssetsOptions);
  const assetById = new Map<string, ExtractedAsset>(assets.map((a) => [a.id, a]));
  for (const a of assets) findings.push(...checkImageQuality(a, profile));

  // 4. Manuscript (with placeholder tokens for non-inline objects)
  const manuscriptLabels: Record<string, string> = {};
  ...
  if (profile.coverPage) {
    ...
    const coverBlocks: BlockNode[] = [
      ...
      para(
        `Prepared for ${profile.label} as a publisher-compliant submission package.`,
      ),
      para("Sincerely,"),
...
function findFigure(doc: CanonicalDocument, id: string): BlockNode | undefined {
...
function findTable(doc: CanonicalDocument, id: string): BlockNode | undefined {
  let found: BlockNode | undefined;
  const visit = (blocks: BlockNode[]): void => {
    for (const b of blocks) {
      if (found) return;
