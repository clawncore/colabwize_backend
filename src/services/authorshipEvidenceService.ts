...
import {
  AuthorshipEvidenceBatchPayload,
  AuthorshipEvidencePayload,
  AuthorshipContributionSummary,
  AuthorshipAnomalyPayload,
} from "../types/authorshipEvidence";

export interface RecordedEvidenceBatchResult {
  batchId: string;
  evidenceIds: string[];
}

export interface ServerObservedEditInput {
  projectId: string;
  userId: string;
  sessionId: string;
...
export interface WritingSessionSnapshotInput {
  projectId: string;
  userId?: string;
  clientSessionId?: string;
  ...
  copyPastes: Array<{
    text?: string;
    time: number;
    copyPasteReason: "largeSingleEvent" | "rapidInsertionBurst";
    copyPasteChars: number;
    copyPasteDurationMs: number;
    copyPasteSourceEdits: number;
  }>;
  metrics?: {
    totalTimeMs: number;
    editCount: number;
...
export class AuthorshipEvidenceService {
  ...
  static async recordEvidenceBatch(
    input: AuthorshipEvidenceBatchPayload
  ): Promise<RecordedEvidenceBatchResult> {
    const normalizedEvidence = input.evidence.map((e) =>
      this.normalizeEvidenceInput(input, e)
    );
    ...
    const batch = await prisma.authorshipEvidenceBatch.create({
      data: {
    ...
    await prisma.authorshipEvidence.createMany({
    ...
    await prisma.authorshipEvidenceBatch.update({
      where: { id: batch.id },
      data: {
        status: "processed",
    ...
    logger.info("Authorship evidence batch recorded", {
      batchId: batch.id,
  ...
  static async recordWritingSessionSnapshot(
    input: WritingSessionSnapshotInput
  ): Promise<RecordedEvidenceBatchResult> {
    if (!input.userId) {
    ...
    const evidence: AuthorshipEvidencePayload[] = [
      {
        projectId: input.projectId,
        userId: input.userId,
  ...
  static async recordServerObservedEdit(
    input: ServerObservedEditInput
  ): Promise<string> {
  ...
  static async recordAnomaly(input: AuthorshipAnomalyPayload): Promise<string> {
    const anomaly = await prisma.authorshipAnomaly.create({
      data: {
        project_id: input.projectId,
    ...
    logger.info("Authorship anomaly recorded", {
  ...
  static async getProjectEvidenceSummary(projectId: string, userId: string) {
  ...
  static async getProjectContributions(projectId: string) {
    const contributions = await prisma.authorshipContribution.findMany({
    ...
    const sessions = await prisma.authorshipCollaborationSession.findMany({
      where: { project_id: projectId },
      select: {
        user_id: true,
    ...
    const sessionMinutes = sessions.reduce<Record<string, number>>((counts, item) => {
      ...
      const minutes = Math.max(
        0,
        Math.round((end.getTime() - item.authenticated_at.getTime()) / 60_000)
      );
  ...
  static async rebuildContributions(projectId: string) {
    const evidence = await prisma.authorshipEvidence.findMany({
      where: { project_id: projectId },
      orderBy: { server_received_at: "asc" },
    });

    const grouped = new Map<string, AuthorshipContributionSummary>();
    ...
    for (const item of evidence) {
      ...
      const existing =
        ...
        } as AuthorshipContributionSummary);
      ...
      existing.offlineEdits +=
        item.payload && typeof item.payload === "object"
          ? Number((item.payload as Record<string, unknown>).offline ?? 0)
          : 0;
  ...
  private static normalizeEvidenceInput(
    batch: AuthorshipEvidenceBatchPayload,
