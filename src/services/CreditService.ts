...
export function calculateCost(
  feature: string,
  metadata?: { wordCount?: number; inputWords?: number; outputWords?: number },
): number {
...
export class CreditService {
   ...
   */
  static async computeBalance(userId: string): Promise<number> {
  ...
  static async getBalance(userId: string): Promise<number> {
    const cached = await prisma.creditBalance.findUnique({
      where: { user_id: userId },
    });
    ...
    if (cachedBalance !== actual) {
      logger.warn("CreditBalance cache drifted; repairing from ledger", {
        userId,
        cachedBalance,
        actual,
      });
  ...
  static async grantCredits(
    ...
    return await prisma.$transaction(async (tx: any) => {
      ...
      if (referenceId) {
        ...
        if (existing) {
          logger.info("Credit grant skipped (idempotent)", {
            userId,
            referenceId,
            type,
          });
  ...
  static async reserveCredits(
    ...
    return await prisma.$transaction(async (tx: any) => {
      ...
      if (referenceId) {
        ...
        if (existing) {
          logger.info("Credit reservation skipped (idempotent)", {
            userId,
            referenceId,
          });
          return existing.id;
      ...
      logger.info("Credits reserved", { userId, cost, referenceId });
      return row.id;
    });
  }
  ...
  static async refundCredits(
    ...
    await prisma.$transaction(async (tx: any) => {
      ...
      if (existing) {
        logger.info("Credit refund skipped (idempotent)", {
          userId,
          referenceId,
        });
        return;
      ...
      logger.info("Credits refunded", { userId, amount, referenceId });
    });
  }
