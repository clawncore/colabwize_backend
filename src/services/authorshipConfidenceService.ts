...
import {
  AuthorshipConfidenceReportPayload,
  ConfidenceDimension,
  RiskDimension,
} from "../types/authorshipEvidence";
import { AuthorshipEvidenceService } from "./authorshipEvidenceService";
...
interface EvidenceTotals {
  ...
  aiAssisted: number;
  collaborationUpdates: number;
  clientTelemetry: number;
...
export class AuthorshipConfidenceService {
  static async generateProjectReport(
    projectId: string,
    userId: string
  ): Promise<AuthorshipConfidenceReportPayload> {
    await AuthorshipEvidenceService.rebuildContributions(projectId);
    ...
    const report: AuthorshipConfidenceReportPayload = {
      projectId,
      userId,
      generatedAt,
      attributionConfidence,
    ...
    const saved = await prisma.authorshipConfidenceReport.create({
      data: {
    ...
    logger.info("Authorship confidence report generated", {
      reportId: saved.id,
      projectId,
      userId,
  ...
  static async getLatestReport(projectId: string, userId: string) {
    const report = await prisma.authorshipConfidenceReport.findFirst({
      where: { project_id: projectId, user_id: userId },
      orderBy: { generated_at: "desc" },
    });

    return report?.report_json as AuthorshipConfidenceReportPayload | null;
  }

  private static async getEvidenceTotals(
    projectId: string,
    userId: string
  ): Promise<EvidenceTotals> {
    const evidence = await prisma.authorshipEvidence.findMany({
      where: { project_id: projectId, user_id: userId },
      select: {
        id: true,
        strength: true,
    ...
    const collaborationSessions = await prisma.authorshipCollaborationSession.count({
      where: { project_id: projectId, user_id: userId },
    });

    const anomalies = await prisma.authorshipAnomaly.count({
      where: { project_id: projectId, user_id: userId },
    });

    const contributors = await prisma.authorshipContribution.count({
    ...
    for (const item of evidence) {
      if (item.strength === "strong") totals.strong += 1;
      if (item.strength === "medium") totals.medium += 1;
      if (item.strength === "weak") totals.weak += 1;
      if (item.source === "hocuspocus" || item.evidence_type === "server_observed_edit") {
        totals.serverObserved += 1;
      }
      if (item.ai_assisted) totals.aiAssisted += 1;
      if (item.evidence_type === "collaboration_update") totals.collaborationUpdates += 1;
      if (item.evidence_type === "client_telemetry") totals.clientTelemetry += 1;
      if (item.evidence_type === "document_snapshot") totals.documentSnapshots += 1;
    }

    return totals;
  }

  private static calculateAttributionConfidence(
    totals: EvidenceTotals
  ): ConfidenceDimension {
    const strongRatio = totals.total > 0 ? totals.strong / totals.total : 0;
    const serverObservedRatio =
      totals.total > 0 ? totals.serverObserved / totals.total : 0;
    const sessionEvidence = Math.min(1, totals.collaborationSessions / 2);
    const score = this.clamp(
      Math.round(
        (strongRatio * 45 + serverObservedRatio * 35 + sessionEvidence * 20) *
  ...
  private static calculateContributionConfidence(
    totals: EvidenceTotals
  ): ConfidenceDimension {
    const coverage = Math.min(1, totals.total / 50);
    const strongSignal = totals.total > 0 ? totals.strong / totals.total : 0;
    const score = this.clamp(
      Math.round((coverage * 55 + strongSignal * 45) * this.volumeMultiplier(totals.total))
    );
  ...
  private static calculateCollaborationClarity(
    totals: EvidenceTotals
  ): ConfidenceDimension {
    const collaborationRatio =
      totals.total > 0 ? totals.collaborationUpdates / totals.total : 0;
    const sessionScore = Math.min(1, totals.collaborationSessions / 2);
    const score = this.clamp(
      Math.round((sessionScore * 60 + collaborationRatio * 40) * this.volumeMultiplier(totals.total))
    );
  ...
  private static calculateEvidenceCompleteness(
    totals: EvidenceTotals
  ): ConfidenceDimension {
    const strongMediumRatio =
      totals.total > 0 ? (totals.strong + totals.medium) / totals.total : 0;
    const volume = Math.min(1, totals.total / 75);
    const snapshotBonus = Math.min(0.15, totals.documentSnapshots * 0.05);
    const score = this.clamp(
      Math.round((strongMediumRatio * 65 + volume * 35 + snapshotBonus * 100) * this.volumeMultiplier(totals.total))
    );
  ...
  private static calculateAITransparency(
    totals: EvidenceTotals
  ): ConfidenceDimension {
    if (totals.total === 0) {
      return {
  ...
  private static async calculateAnomalyRisk(
    ...
    const anomalies = await prisma.authorshipAnomaly.findMany({
      where: { project_id: projectId, user_id: userId },
      orderBy: { score: "desc" },
      take: 20,
    });

    const maxSeverityScore = anomalies.reduce(
      (max: number, anomaly: Prisma.AuthorshipAnomalyUncheckedCreateInput) =>
        Math.max(max, Number(anomaly.score ?? 0)),
      0
    );
    const highOrCritical = anomalies.filter(
      (anomaly: Prisma.AuthorshipAnomalyUncheckedCreateInput) =>
    ...
    return {
      ...
      flags: anomalies.map((anomaly: Prisma.AuthorshipAnomalyUncheckedCreateInput) => ({
  ...
  private static calculateOverallReliability(dimensions: {
    ...
    const mean =
      (dimensions.attributionConfidence.score +
  ...
  private static buildLimitations(totals: EvidenceTotals): string[] {
    const limitations = [
  ...
  private static volumeMultiplier(total: number): number {
    if (total === 0) return 0;
    if (total < 5) return 0.55;
    if (total < 20) return 0.75;
    return 1;
  }

  private static labelFromScore(score: number): "High" | "Medium" | "Low" | "Insufficient" {
  ...
  private static interval(low: number, high: number): [number, number] {
