...
export class CitationRoleClassifier {
  /**
   * Classify a single citation based on its surrounding context.
   */
  static classify(text: string, context?: string): ClassifiedCitation {
    const sourceText = context || text;

    let bestMatch: { role: CitationRole; confidence: number; pattern?: string } = {
      role: "UNKNOWN",
      confidence: 0,
    };
  ...
  static classifyBatch(
    citations: Array<{ text: string; context?: string }>
  ): ClassifiedCitation[] {
   ...
   */
  static summarize(classified: ClassifiedCitation[]): Record<CitationRole, number> {
    ...
    return summary as Record<CitationRole, number>;
  }
   ...
   */
  private static computeMatchQuality(matchText: string, sourceText: string): number {
    const matchIndex = sourceText.indexOf(matchText);
    if (matchIndex < 0) return 0.8;

    const citationAnchor = this.findCitationAnchor(sourceText);
    if (citationAnchor < 0) return 0.8;
   ...
   */
  private static findCitationAnchor(text: string): number {
    const markerMatch = text.match(/\[\d+\]|\([^)]*\d{4}[^)]*\)/);
    if (markerMatch) return markerMatch.index ?? -1;
    const authorMatch = text.match(/[A-Z][a-z]+(?:\s+et\s+al\.?)?(?:\s*\(|,\s*)/);
    if (authorMatch) return authorMatch.index ?? -1;
    return -1;
  }
}

const ALL_ROLES: CitationRole[] = [
  "FOUNDATIONAL",
  "DATA_SOURCE",
  "METHODOLOGICAL",
  "CONTEXTUAL",
