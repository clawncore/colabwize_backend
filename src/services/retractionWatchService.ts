...
export interface RetractionInfo {
  ...
  retractionType?: "RETRACTION" | "EXPRESSION_OF_CONCERN" | "CORRECTION";
  hasExpressionOfConcern: boolean;
  eocDate?: string;
  hasCorrection: boolean;
  correctionDOI?: string;
  isHijacked: boolean;
  source: "crossref" | "retraction_watch" | "hijacked_checker";
  title?: string;
 ...
 */
export class RetractionWatchService {
   ...
   */
  static async checkDOI(doi: string): Promise<RetractionInfo | null> {
    if (!doi || doi.trim().length === 0) return null;
    ...
    try {
      ...
      logger.warn("[RetractionWatch] Failed to check DOI", { doi, error });
      return null;
    }
  }
   ...
   */
  static async checkDOIs(dois: string[]): Promise<Map<string, RetractionInfo | null>> {
    const results = new Map<string, RetractionInfo | null>();
   ...
   */
  private static async queryCrossRefRetractions(doi: string): Promise<RetractionInfo | null> {
    try {
      ...
      const updates: any[] = msg["update-to"] || [];
      const title = Array.isArray(msg.title) ? msg.title[0] : msg.title;

      const retractionUpdate = updates.find(
        (u: any) => u.type === "retraction" || u.type === "withdrawal"
      );
      const eocUpdate = updates.find(
        (u: any) => u.type === "expression-of-concern"
      );
      const correctionUpdate = updates.find(
        (u: any) => u.type === "correction" || u.type === "erratum"
      );
   ...
   */
  static async checkHijackedJournal(journalName: string): Promise<boolean> {
    if (!journalName || journalName.trim().length < 3) return false;

    const normalized = journalName.trim().toLowerCase();
    const cacheKey = `hijacked:${this.cacheKey(normalized)}`;

    const cached = this.cache.get(cacheKey);
    if (cached) return cached.info?.isHijacked ?? false;
   ...
   */
  private static readonly HIJACKED_JOURNAL_PATTERNS = [
  ...
  private static matchHijackedJournal(name: string): boolean {
    return this.HIJACKED_JOURNAL_PATTERNS.some((pattern) => pattern.test(name));
  }

  private static cacheKey(input: string): string {
