 ...
 *
 * Every billable feature in ColabWize MUST execute through this gateway. No
 * feature is executable unless it passes through here.
 *
 * Lifecycle (the only allowed one):
 *
 *   hold  ->  execute feature  ->  confirm   (success)
 *     \->  release                         (failure / timeout)
 *
 * The gateway writes an immutable row to the UsageEvent ledger on every hold.
 * userEntitlement.features is treated as a read-model cache; the UsageEvent
...
export class BillingError extends Error {
  code: "PLAN_LIMIT_REACHED" | "INSUFFICIENT_CREDITS" | "ENTITLEMENT_ERROR";
...
export class BillingGateway {
  ...
  static async hold(
    userId: string,
    feature: string,
    metadata?: { wordCount?: number; inputWords?: number; outputWords?: number; referenceId?: string },
  ): Promise<BillingHold> {
    ...
    if (referenceId) {
      let existing;
      try {
        existing = await prisma.usageEvent.findFirst({
        ...
        logger.error("UsageEvent table query failed — migration may be pending", {
          error: e.message,
        });
      }
      if (existing && (existing.status === "HELD" || existing.status === "CONSUMED")) {
        return {
          eventId: existing.id,
          source: existing.source as "PLAN" | "CREDIT",
          cost: existing.cost,
          remaining: -1,
        };
      }
    ...
    const result = await prisma.$transaction(async (tx: any) => {
      ...
      if (!ent) throw new BillingError("ENTITLEMENT_ERROR", "Entitlements not found");
      ...
      const resolveRights = async () => {
        ...
        if (!latest) throw new BillingError("ENTITLEMENT_ERROR", "Entitlements not found");
        ent = latest;
      ...
      for (let attempt = 0; attempt <= MAX_HOLD_RETRIES; attempt++) {
        ...
        if (rights && (rights.unlimited || rights.remaining > 0)) {
          if (rights.unlimited) {
            ...
            return { event, source: "PLAN" as const, cost: 0, remaining: -1 };
          }
   ...
   */
  static async confirm(eventId: string, metadata?: Record<string, any>): Promise<void> {
    try {
      ...
      logger.error("Billing confirm failed", { eventId, error: e.message });
    }
  }
   ...
   */
  static async release(eventId: string, reason?: string): Promise<void> {
    try {
      const event = await prisma.usageEvent.findUnique({ where: { id: eventId } });
      if (!event || event.status !== "HELD") return;
      ...
      logger.info("Billing hold released", { eventId, source: event.source, cost: event.cost, reason });
    } catch (e: any) {
      logger.error("Billing release failed", { eventId, error: e.message });
    }
  }
   ...
   */
  static async withFeature<T>(
    userId: string,
    feature: string,
    metadata: { wordCount?: number; inputWords?: number; outputWords?: number; referenceId?: string } | undefined,
    execute: () => Promise<T>,
  ): Promise<T> {
    ...
    try {
      ...
      await this.release(hold.eventId, (e as Error)?.message);
      throw e;
    }
  }
}
...
async function getPlanForEntitlement(tx: any, userId: string): Promise<string> {
  const sub = await tx.subscription.findUnique({ where: { user_id: userId } });
  if (!sub) return "free";
  if (sub.entitlement_expires_at && new Date() > sub.entitlement_expires_at) return "free";
...
function prismaJsonMerge(metadata: Record<string, any>): Record<string, any> {
