...
import { MendeleyService } from '../../services/mendeleyService.js';
import {
  normalizeZoteroItem,
  normalizeMendeleyItem,
  normalizeManualInput,
} from '../../utils/citationNormalizer.js';
import type { CitationProvider } from '../../models/CanonicalCitation.js';
...
function asyncHandler(
  ...
  return (req: Request, res: Response) => {
    fn(req, res).catch((err: any) => {
      logger.error(`[References API] Unhandled error: ${err?.message}`, err);
      if (!res.headersSent) {
...
function parseProvider(value: unknown): CitationProvider | null {
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const citations = await prisma.citation.findMany(findOptions);
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        ...
        switch (provider) {
          case 'zotero':
            normalized = normalizeZoteroItem(rawItem, userId);
            break;
          case 'mendeley':
            normalized = normalizeMendeleyItem(rawItem, userId);
            break;
          case 'manual':
          case 'crossref':
          default:
            normalized = normalizeManualInput(rawItem);
            break;
        }
        ...
        logger.error(`[References API] Import error at index ${i}: ${err?.message}`);
      }
    }
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const lastCollection = await prisma.referenceCollection.findFirst({
      where: { project_id: String(projectId), user_id: userId },
      orderBy: { sort_order: 'desc' },
    });

    const collection = await prisma.referenceCollection.create({
      data: {
        project_id: String(projectId),
        user_id: userId,
        name,
...
router.post(
  '/collections/:collectionId/citations',
  authenticateHybridRequest,
  asyncHandler(async (req: Request, res: Response) => {
    const userId = getUserId(req);
    ...
    // Determine sort order (append to end)
    const lastEntry = await prisma.collectionCitation.findFirst({
      where: { collection_id: collectionId },
      orderBy: { sort_order: 'desc' },
    });

    try {
      const entry = await prisma.collectionCitation.create({
        data: {
...
router.delete(
  '/collections/:collectionId/citations/:citationId',
  authenticateHybridRequest,
  asyncHandler(async (req: Request, res: Response) => {
    const userId = getUserId(req);
    ...
    try {
      await prisma.collectionCitation.delete({
        where: {
